...
class ResultsPage(QWidget):
    ...
    def populate(self):
        ...
        for record in reversed(self.records):
            ...
            if self.audio_scope.isChecked():
                searchable.extend(s['text'] for s in record.get('segments', []))
            visual_matches = self.matching_visual_frames(record, query)
            if visual_matches:
                self.visual_matches[record['id']] = visual_matches
            if self.visual_scope.isChecked():
                selected_layer = self.layer.currentData()
                for frame in record.get('visual_index', {}).get('frames', []):
                    keywords = frame.get('keywords', {})
                    values = (keywords.get(selected_layer, []) if selected_layer else
                              [word for words in keywords.values() for word in words])
                    searchable.extend(values)
                    searchable.extend(frame.get('search_words', []))
            direct, embedded = query_hit_counts(searchable, query)
            if query and not (direct or embedded):
                continue
    ...
    def select(self, item, previous=None):
        ...
        if self.transcript.count() and self.transcript_search.text():
...
class LegacyProjectsPage(QWidget):
    ...
    def insert_selected_media(self):
        item = self.media_bin.currentItem()
        if item:
            self.add_media_to_timeline(item.data(Qt.UserRole))
    ...
    def add_media_to_timeline(self, video, start=None, track_index=None):
        ...
        if linked:
            audio_index = next((i for i, track in enumerate(tracks)
                                if track.get('type') == 'audio' and not track.get('locked')), None)
...
class ProjectsPage(LegacyProjectsPage):
    """Project media, source inspection, multitrack sequences, and export queue."""
    def __init__(self, window):
        ...
        heading = QHBoxLayout()
        words = QVBoxLayout()
        words.addWidget(label('PROJECT EDITING ROOM', 'eyebrow'))
        words.addWidget(label('Projects', 'title'))
        words.addWidget(label('Organize sources, build story sequences, and hand finished timelines to Premiere.', 'subtle'))
        heading.addLayout(words)
        heading.addStretch()
        heading.addWidget(button('+ New project', self.window.new_project, True))
        layout.addLayout(heading)
        ...
        workspace.addWidget(self.build_media_panel())
        workspace.addWidget(self.build_editing_panel())
        workspace.addWidget(self.build_export_panel())
        workspace.setSizes([280, 740, 225])
    ...
    def build_media_panel(self):
        ...
        ll.addWidget(label('SAVED PROJECTS', 'eyebrow'))
        self.projects = QListWidget()
        self.projects.setMaximumHeight(112)
        self.projects.currentItemChanged.connect(self.project_changed)
        ll.addWidget(self.projects)
        project_actions = QHBoxLayout()
        project_actions.addWidget(button('Rename', self.window.rename_project))
        remove = button('Remove', self.window.remove_project)
        remove.setObjectName('danger')
        project_actions.addWidget(remove)
        ll.addLayout(project_actions)
        self.name = label('Select a project')
        ...
        self.add_folders = button('+ Folder', self.window.add_project_folders)
        self.add_files = button('+ Video', self.window.add_project_files)
        ...
        source_actions.addWidget(self.add_folders)
        ...
        ll.addWidget(label('DRAG INTO TIMELINE', 'eyebrow'))
        self.media_bin = timeline.MediaBinList()
        self.media_bin.setDragEnabled(True)
        self.media_bin.setMaximumHeight(105)
        self.media_bin.itemClicked.connect(lambda item: self.inspect_source(item.data(Qt.UserRole)))
        self.media_bin.itemDoubleClicked.connect(lambda item: self.add_media_to_timeline(item.data(Qt.UserRole)))
        ll.addWidget(self.media_bin)
        ll.addWidget(button('Insert selected into sequence', self.insert_selected_media))
        self.status = label('Create a project, then add folders or videos.', 'subtle')
        self.status.setWordWrap(True)
    ...
    def build_editing_panel(self):
        ...
        source_layout.addWidget(label('SOURCE MONITOR', 'eyebrow'))
        self.source_name = label('Select media from the project bin', 'subtle')
        source_layout.addWidget(self.source_name)
        self.video = QVideoWidget()
        self.video.setMinimumHeight(105)
        self.player = QMediaPlayer(self)
        self.audio = QAudioOutput(self)
        self.audio.setVolume(0.75)
        self.player.setAudioOutput(self.audio)
        self.player.setVideoOutput(self.video)
        source_layout.addWidget(self.video, 1)
        ...
        transcript_layout.addWidget(label('TRANSCRIPT / NOTES', 'eyebrow'))
        self.source_transcript = QTextBrowser()
        self.source_transcript.setPlaceholderText('Processed transcript text appears here.')
        transcript_layout.addWidget(self.source_transcript)
        inspection.addWidget(transcript_card)
        inspection.setSizes([390, 390])
        ...
        for text, command_name in (('Undo', 'undo'), ('Redo', 'redo'), ('Split', 'split'),
                                   ...
                                   ('Duplicate clip', 'duplicate'), ('Delete', 'delete')):
            tools.addWidget(button(text, lambda checked=False, cmd=command_name: self.sequence_command(cmd)))
        tools.addStretch()
        cl.addLayout(tools)
        track_tools = QHBoxLayout()
        track_tools.addWidget(button('+ V track', lambda: self.add_track('video')))
        track_tools.addWidget(button('+ A track', lambda: self.add_track('audio')))
        track_tools.addWidget(button('Track on/off', lambda: self.sequence_command('toggle_track')))
        track_tools.addWidget(button('Lock', lambda: self.sequence_command('lock_track')))
        track_tools.addWidget(button('Solo', lambda: self.sequence_command('solo_track')))
        track_tools.addWidget(button('+ Marker', lambda: self.sequence_command('marker')))
        ...
        self.zoom.valueChanged.connect(lambda value: self.timeline_canvas.set_zoom(value))
        track_tools.addWidget(self.zoom)
        track_tools.addWidget(button('Fit', self.fit_timeline))
    ...
    def build_export_panel(self):
        ...
        layout.addWidget(button('Queue active sequence', self.queue_sequence))
        export_xml = button('Export Premiere XML', self.export_sequence, True)
        export_xml.setToolTip('Write an FCP 7 XML timeline for import into Premiere Pro')
        layout.addWidget(export_xml)
        layout.addWidget(button('Collect used media', self.collect_media))
        layout.addWidget(button('Relink offline media', self.relink_offline_media))
        layout.addWidget(button('Remove from queue', self.remove_export_item))
    ...
    def project_changed(self, current, previous=None):
        ...
        self.populate_export_queue()
        if self.current() and not self.window.batch:
            self.status.setText('Ready. Drag media into a sequence or process unprepared sources.')

    def populate_tree(self):
        project = self.current()
        self.tree.blockSignals(True)
        self.tree.clear()
        self.media_bin.clear()
        self.item_map.clear()
        if not project:
            self.name.setText('Select or create a project')
            self.summary.setText('Projects now contain media, sequences, and an export queue.')
        else:
            timeline.ensure_project_editing(project)
            self.name.setText(project['name'])
            for folder in project.get('folders', []):
                ...
                for video in folder.get('files', []):
                    ...
                    if video.get('status') != 'Unreadable':
                        ...
                        media_item = QListWidgetItem(video['name'])
                        media_item.setToolTip(video['path'])
                        media_item.setData(Qt.UserRole, payload)
                        self.media_bin.addItem(media_item)
                parent.setExpanded(True)
        self.tree.blockSignals(False)
        self.update_summary()
    ...
    def update_summary(self):
        ...
        if project:
            sequence = self.active_sequence()
            sequence_text = (f" · {sequence['name']} {duration_text(timeline.sequence_duration(sequence))}"
                             if sequence else '')
            self.summary.setText(f"{len(files)} media · {len(selected)} selected · {len(project.get('sequences', []))} sequences{sequence_text}")
            if not self.window.batch:
                self.progress_detail.setText(f"{len(selected)} {'video' if len(selected) == 1 else 'videos'} ready")
        ready = bool(project and selected and not self.window.batch and not self.window.scanning)
        self.process.setEnabled(ready)
    ...
    def set_busy(self, busy):
        for widget in (self.projects, self.tree, self.media_bin, self.add_folders, self.add_files, self.remove_source):
            widget.setEnabled(not busy)
        if not busy:
            self.update_summary()
...
class Window(QMainWindow):
    def __init__(self):
        ...
        side.addSpacing(18)
        self.nav = []
        navigation = (('Queue', 'queue'), ('Projects', 'projects'), ('Library', 'library'), ('Settings', 'settings'))
        for i, (name, icon_name) in enumerate(navigation):
            b = QToolButton()
            b.setText(name)
            b.setIcon(tile_icon(icon_name))
            b.setIconSize(QSize(28, 23))
            b.setToolButtonStyle(Qt.ToolButtonTextUnderIcon)
            b.setFixedHeight(61)
            b.clicked.connect(lambda checked=False, index=i: self.navigate(index))
            b.setObjectName('nav')
            b.setCheckable(True)
            b.setToolTip(('Folder queue', 'Project editing room', 'Video library', 'Settings')[i])
            self.nav.append(b)
            side.addWidget(b)
        side.addStretch()
        self.usage_labels = {}
        ...
        side.addSpacing(6)
        offline_heading = label('LOCAL', 'eyebrow')
        offline_heading.setAlignment(Qt.AlignCenter)
        side.addWidget(offline_heading)
        privacy = label('Offline mode\nLocal processing', 'subtle')
        privacy.setWordWrap(True)
        privacy.setAlignment(Qt.AlignCenter)
        privacy.setStyleSheet('font-size: 9px;')
        side.addWidget(privacy)
        side.addSpacing(10)
        version = label('Tracer · 1.14', 'subtle')
    ...
    def make_queue(self):
        ...
        self.add_button.setToolTip('Select folders, videos, or both in one window')
    ...
    def closeEvent(self, event):
        ...
        self.projects_page.player.stop()
        self.save()
        event.accept()
