...
from PySide6.QtGui import QColor, QDrag, QFont, QPainter, QPen, QBrush, QKeySequence
from PySide6.QtWidgets import QListWidget, QTabBar, QWidget
...
def write_final_cut_xml(sequence, path):
    ...
    target.write_text(final_cut_xml(sequence), encoding='utf-8')
    return target
...
class TimelineCanvas(QWidget):
    ...
    def set_zoom(self, value):
        self.pixels_per_second = max(2.0, min(80.0, float(value)))
        self._resize_canvas()
        self.update()
    ...
    def paintEvent(self, event):
        ...
        painter.fillRect(0, 0, self.width(), RULER_HEIGHT, QColor('#20201f'))
        painter.fillRect(0, 0, TRACK_HEADER, self.height(), QColor('#1d1d1c'))
        if not self.sequence:
        ...
        for index, track in enumerate(self.sequence.get('tracks', [])):
            ...
            painter.fillRect(0, y, self.width(), TRACK_HEIGHT,
                             QColor('#1b1b1a' if index % 2 else '#181818'))
            ...
            for clip in track.get('clips', []):
                ...
                painter.setPen(QColor('#101010') if selected else QColor('#f6f2eb'))
                painter.setFont(QFont('Segoe UI', 8, QFont.DemiBold))
                painter.drawText(round(x + 7), round(cy + 19), clip.get('name', 'Clip'))
                painter.setFont(QFont('Segoe UI', 7))
                painter.drawText(round(x + 7), round(cy + 36),
    ...
    def mousePressEvent(self, event):
        ...
        if clip:
            ...
            self._drag = (event.position().x(), index, clip,
                          float(clip.get('start', 0)), float(clip.get('end', 0)))
        else:
            self.selected_id = None
    ...
    def mouseMoveEvent(self, event):
        if not self._drag or not (event.buttons() & Qt.LeftButton) or not self.sequence:
            return
        press_x, old_index, clip, old_start, old_end = self._drag
        track = self.sequence['tracks'][old_index]
        if track.get('locked'):
            return
        delta = (event.position().x() - press_x) / self.pixels_per_second
        duration = old_end - old_start
        clip['start'] = round(max(0.0, old_start + delta), 4)
        clip['end'] = round(clip['start'] + duration, 4)
        ...
        if 0 <= target < len(self.sequence['tracks']) and target != old_index:
            ...
            if destination.get('type') == track.get('type') and not destination.get('locked'):
                ...
                self._drag = (press_x, target, clip, old_start, old_end)
        self._resize_canvas()
        self.update()

    def mouseReleaseEvent(self, event):
        if self._drag:
            _, _, clip, old_start, _ = self._drag
