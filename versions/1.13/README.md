...
Double-click **Launch Tracer 1.13.vbs** in this folder to open this exact source snapshot. A future export build will be created as **dist/Tracer/Tracer.exe**.
...
The editable 1.12 source remains preserved under **versions/1.12**. The 1.13 installer is intentionally deferred until an exportable build is requested; completed earlier TransPro installers remain in **release**.
...
9. Results appear as videos first. Choosing one slides the browser away, replaces the page heading with the selected filename, and leaves a transcript-first workspace, source preview, and full-width frame gallery. Matching search words are highlighted directly in the displayed transcript. **See All** slides the list over the left edge of the workspace without resizing the media dashboard. Drag the divider between transcript and preview left or right to suit the footage. Compact arrow buttons page through frames. **Show in Explorer** opens the original video's location with that exact file highlighted.
...
## Unified offline search in 1.13
