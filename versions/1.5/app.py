...
from PySide6.QtCore import (Qt, QThread, Signal, QTimer, QUrl, QSize, QLockFile, QEvent,
...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
    ...
    QInputDialog, QButtonGroup, QToolButton, QToolTip)
...
QPushButton#processStart { background: #9fc7b0; color: #142219; border: 1px solid #bddbc9; padding: 10px 22px; border-radius: 5px; font-weight: 700; }
QPushButton#processStart:hover { background: #b5d8c3; border-color: #d4eadc; }
...
QPushButton#processCancel { background: transparent; color: #f40009; border: 0; padding: 0; font-size: 27px; font-weight: 500; }
...
def tile_icon(kind):
    ...
    colour = icon_colours.get(kind, '#d9a68f')
    ...
    if kind == 'audio':
        ...
        points = ((9, 18), (20, 7), (35, 12), (31, 27), (17, 26))
...
class ChoiceTile(QToolButton):
    ...
    def _apply_style(self):
        ...
        palettes = {
            'audio': ((67, 48, 82), (203, 179, 229)),
            'frame': ((38, 73, 87), (155, 204, 224)),
            'ai': ((42, 79, 57), (174, 218, 190)),
        }
        selected_bg, selected_border = palettes.get(self._icon_kind, ((62, 43, 36), (211, 119, 87)))
        accent = '#%02x%02x%02x' % selected_border
        bg = '#%02x%02x%02x' % tuple(mix(a, b) for a, b in zip((29, 29, 28), selected_bg))
        border = '#%02x%02x%02x' % tuple(mix(a, b) for a, b in zip((57, 55, 52), selected_border))
        foreground = '#%02x%02x%02x' % tuple(mix(a, b) for a, b in zip((166, 162, 155), (239, 238, 235)))
        self.setStyleSheet(
            f'QToolButton {{ background:{bg}; color:{foreground}; border:1px solid {border}; '
            f'border-radius:5px; padding:{"5px" if self._icon_only else "7px"}; font-size:11px; font-weight:600; }} '
            f'QToolButton:hover {{ border-color:{accent}; background:#242422; }} '
...
class OptionsDialog(QDialog):
    def __init__(self, options, folders, parent, batch_name='folder batch'):
        ...
        layout.addWidget(label('Process ' + batch_name, 'title'))
        ...
        transcript_layout.setContentsMargins(16, 13, 16, 13)
        transcript_layout.setSpacing(8)
        transcript_layout.addWidget(label('TRANSCRIPT & OUTPUT', 'eyebrow'))
        left_form = QFormLayout()
        left_form.setSpacing(8)
        transcript_layout.addLayout(left_form)
        capture_card = QFrame()
        capture_card.setObjectName('card')
        capture_layout = QVBoxLayout(capture_card)
        capture_layout.setContentsMargins(16, 13, 16, 13)
        capture_layout.setSpacing(8)
        capture_layout.addWidget(label('FRAMES & VISUAL INDEX', 'eyebrow'))
        right_form = QFormLayout()
        right_form.setSpacing(8)
        capture_layout.addLayout(right_form)
        settings_columns.addWidget(transcript_card, 1)
        settings_columns.addWidget(capture_card, 1)
        layout.addLayout(settings_columns, 1)
        self.model = QComboBox()
        for key, info in core.MODEL_INFO.items():
            ready = core.model_ready(key)
            self.model.addItem(info[0] + ('' if ready else ' · Download in Settings'), key)
            if not ready:
                self.model.model().item(self.model.count() - 1).setEnabled(False)
        self.model.setCurrentIndex(list(core.MODEL_INFO).index(options['model']))
        self.model_title = label('Whisper model')
        left_form.addRow(self.model_title, self.model)
        self.audio_info = label('Audio analysis · 16 kHz mono, prepared automatically for Whisper', 'subtle')
        self.audio_info.setToolTip('Your original audio is not changed. Whisper receives a temporary 16 kHz mono signal internally.')
        self.audio_info.setWordWrap(True)
        left_form.addRow('', self.audio_info)

        mode_box = QWidget()
        mode_box.setObjectName('transparentRow')
        mode_row = QHBoxLayout(mode_box)
        mode_row.setContentsMargins(0, 0, 0, 0)
        self.interval_mode = ChoiceTile('interval', 'Fixed interval', compact=True)
        self.scene_mode = ChoiceTile('scene', 'Scene changes', compact=True)
        self.mode_group = QButtonGroup(self)
        self.mode_group.addButton(self.interval_mode)
        self.mode_group.addButton(self.scene_mode)
        (self.interval_mode if options['mode'] == 'interval' else self.scene_mode).setChecked(True)
        (self.interval_mode if options['mode'] == 'interval' else self.scene_mode).set_tone(1)
        self.interval_mode.setToolTip('Save a frame at the exact interval selected below.')
        self.scene_mode.setToolTip('Compare frames and save at strong visual changes, usually edits or shot cuts.')
        mode_row.addWidget(self.interval_mode)
        mode_row.addWidget(self.scene_mode)
        self.screenshot_mode_title = label('Screenshot mode')
        right_form.addRow(self.screenshot_mode_title, mode_box)
        ...
        interval_row.addWidget(self.slider)
        interval_row.addWidget(self.interval)
        self.interval_title = label('Capture every')
        right_form.addRow(self.interval_title, self.interval_box)
        ...
        right_form.addRow(self.visual_interval_title, self.visual_interval_box)

        size_box = QWidget()
        size_box.setObjectName('transparentRow')
        ...
        self.width.setValue([640, 1280, 1920, 0].index(options['width']))
        ...
        right_form.addRow(self.size_title, size_box)

        format_box = QWidget()
        format_box.setObjectName('transparentRow')
        format_row = QHBoxLayout(format_box)
        format_row.setContentsMargins(0, 0, 0, 0)
        self.jpeg = button('JPEG · Smaller')
        self.png = button('PNG · Lossless')
        ...
        self.jpeg.setToolTip('Smaller files with adjustable visual quality. Best for most contact sheets.')
        self.png.setToolTip('Lossless images with larger files. PNG has no quality-loss setting.')
        format_row.addWidget(self.jpeg)
        format_row.addWidget(self.png)
        format_row.addStretch()
        self.format_title = label('Image format')
        right_form.addRow(self.format_title, format_box)
        ...
        self.quality_label.setMinimumWidth(120)
        quality_row.addWidget(self.quality, 1)
        quality_row.addWidget(self.quality_label)
        self.quality_title = label('Image quality')
        right_form.addRow(self.quality_title, quality_box)
        ...
        left_form.addRow(self.language_title, language_box)

        self.device = QComboBox()
        for text, value in [('Automatic · GPU preferred', 'auto'), ('NVIDIA GPU only', 'cuda'), ('CPU', 'cpu')]:
            self.device.addItem(text, value)
        self.device.setCurrentIndex(self.device.findData(options['device']))
        self.device_title = label('Processing device')
        left_form.addRow(self.device_title, self.device)
        ...
        left_form.addRow('', self.redo)
        location_box = QWidget()
        location_box.setObjectName('transparentRow')
        location = QHBoxLayout(location_box)
        location.setContentsMargins(0, 0, 0, 0)
        self.output = QLineEdit(options.get('output', ''))
        self.output.setReadOnly(True)
        self.output.setPlaceholderText('Inside each selected folder / TransPro Results')
        location.addWidget(self.output)
        location.addWidget(button('Browse', self.choose_output))
        location.addWidget(button('Reset', self.output.clear))
        self.output_title = label('Save results')
        left_form.addRow(self.output_title, location_box)

        self.estimate_label = label('')
        self.estimate_label.setWordWrap(True)
        layout.addWidget(self.estimate_label)

        layout.addWidget(label('CHOOSE WHAT TRANSPRO SHOULD CREATE', 'eyebrow'), alignment=Qt.AlignHCenter)
        ...
        self.cancel_button.setFixedSize(48, 48)
        row.addWidget(self.cancel_button)
        self.start = button('Start processing', self.submit)
        ...
        for widget in (self.interval_mode, self.scene_mode, self.jpeg, self.png,
    ...
    def values(self):
        return {**self.options, 'model': self.model.currentData(),
                'mode': 'interval' if self.interval_mode.isChecked() else 'scene',
    ...
    def update_estimate(self):
        ...
        self.audio_info.setVisible(True)
        self.audio_info.setEnabled(o['transcribe'])
        ...
        for title in (self.screenshot_mode_title, self.size_title, self.format_title,
                      self.quality_title):
            title.setEnabled(o['screenshots'])
        for widget in (self.interval_mode, self.scene_mode, self.width, self.jpeg, self.png):
        ...
        if not valid:
            self.estimate_label.setText('Choose at least one output. Install any missing transcription or visual-search model in Settings.')
            return
        e = core.estimate(self.files, o)
        note = ('Scene estimate assumes one cut every 5 seconds.' if o['screenshots'] and o['mode'] == 'scene'
                else ('Frame count may vary with the video frame rate.' if o['screenshots'] else ''))
        visual = f" · visual index every {o['visual_interval']:.0f}s" if o['visual_index'] else ''
        image_note = f" · {o['image_format'].upper()}" if o['screenshots'] else ''
        self.estimate_label.setText(f"{e['videos']} videos · {duration_text(e['duration'])} total · ≈ {e['count']:,} screenshots{image_note}{visual}\nEstimated output: {size_text(e['bytes'])} (rough guide; image content affects size). {note}")
...
class ResultsPage(QWidget):
    def __init__(self, parent=None):
        ...
        close_search = button('×', self.collapse_search)
        close_search.setObjectName('danger')
        close_search.setFixedSize(32, 30)
        close_search.setToolTip('Close video browser')
        search_heading.addWidget(close_search)
        ll.addLayout(search_heading)
        self.search = QLineEdit()
        self.search.setPlaceholderText('Search videos or transcript text…')
        self.search.textChanged.connect(self.populate)
        ...
        self.library_split.addWidget(self.search_panel)
        right = QWidget()
        rl = QVBoxLayout(right)
        rl.setContentsMargins(10, 0, 0, 0)
        rl.setSpacing(10)
        toolbar = QHBoxLayout()
        self.browse_videos = button('Browse videos', self.open_search)
        toolbar.addWidget(self.browse_videos)
        self.name = label('Select a completed video', 'eyebrow')
        toolbar.addWidget(self.name)
        toolbar.addStretch()
        ...
        upper.addWidget(transcript_panel, 3)

        video_panel = QFrame()
        video_panel.setObjectName('card')
        video_panel.setMinimumWidth(245)
        video_panel.setMaximumWidth(340)
        ...
        upper.addWidget(video_panel, 1)
        rl.addLayout(upper, 2)
        ...
        self.library_split.addWidget(right)
        self.library_split.setSizes([290, 900])
        self.library_split.setStretchFactor(1, 1)
        layout.addWidget(self.library_split, 1)
        self.search_animation = QPropertyAnimation(self.search_panel, b'maximumWidth', self)
    ...
    def populate(self):
        ...
        for record in reversed(self.records):
            searchable = (record['source'] + ' ' + record.get('project_name', '') + ' ' +
                          ' '.join(s['text'] for s in record.get('segments', [])) + ' ' +
                          ' '.join(record.get('visual_index', {}).get('search_words', [])))
            if query and query not in searchable.casefold():
                continue
            project = (' · ' + record['project_name']) if record.get('project_name') else ''
            item = QListWidgetItem(f"{Path(record['source']).name}\n{duration_text(record['duration'])} · {len(record.get('frames', [])):,} frames{project}")
            item.setData(Qt.UserRole, record)
    ...
    def collapse_search(self):
        ...
        self.search_animation.setStartValue(max(1, self.search_panel.width()))
    ...
    def open_search(self):
        ...
        self.library_split.setSizes([310, max(600, self.library_split.width() - 310)])
        self.search_target = 310
        self.search_animation.setStartValue(0)
        self.search_animation.setEndValue(310)
        self.search_animation.start()
    ...
    def fill_frames(self):
        ...
        self.page_label.setText(f'{self.frame_offset + 1 if frames else 0}–{min(self.frame_offset + 60, len(frames))} of {len(frames):,}')

    def page_frames(self, direction):
        if self.record:
            n = len(self.record.get('frames', []))
...
class Window(QMainWindow):
    def __init__(self):
        ...
        brand.addWidget(label('TransPro', 'brand'))
        brand.addStretch()
        side.addLayout(brand)
        side.addWidget(label('TRANSCRIPTION DESK', 'eyebrow'))
        side.addSpacing(30)
        self.nav = []
        for i, name in enumerate(('Folder queue', 'Project dashboard', 'Footage search', 'Video library', 'Settings')):
        ...
        side.addSpacing(15)
        side.addWidget(label('TransPro · 1.5', 'subtle'))
        outer.addWidget(sidebar)
        self.pages = QStackedWidget()
        self.pages.addWidget(self.make_queue())
        self.projects_page = ProjectsPage(self)
        self.pages.addWidget(self.projects_page)
        self.search_page = FootageSearchPage(self)
        self.search_page.set_records(self.state['results'])
    ...
    def navigate(self, index):
        ...
        if index != 2 and hasattr(self, 'search_page'):
            self.search_page.player.pause()
        if index != 3 and hasattr(self, 'results'):
    ...
    def make_settings(self):
        ...
        for key, info in core.MODEL_INFO.items():
            ...
            row.setContentsMargins(19, 16, 19, 16)
            words = QVBoxLayout()
            words.addWidget(label(info[0]))
            state = label('', 'subtle')
    ...
    def refresh_models(self):
        ...
        if hasattr(self, 'visual_model_state'):
            ...
            self.visual_model_state.setText(('Installed · ' if ready else 'Included with 1.5 · approximately ') + size_text(size))
...
def main():
    ...
    configure_fonts()
    app.setApplicationName('TransPro')
    app.setOrganizationName('TransPro')
    ...
    if not lock.tryLock(0):
        QMessageBox.information(None, 'TransPro is open', 'TransPro is already running. Switch to its existing window.')
        return 0
    app.setStyle('Fusion')
