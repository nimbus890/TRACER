...
The editable 1.5 source is preserved under **versions/1.5**. Its installer is intentionally deferred until an exportable build is requested; completed earlier installers remain in **release**.
...
4. Click **Process folders**. Choose a model, language, device and outputs.
5. Choose any combination of the three compact, opaque icon tiles above Start Processing. Hover an icon for its Transcript, Screenshots, or AI Visual Index explanation; unrelated settings fade when that output is off.
6. For screenshots, choose a 0.1–30 second interval (type an exact value or use the slider), or choose scene changes. Set the resolution slider and choose adjustable-quality JPEG or lossless PNG.
7. Review the approximate image count and size. Choose an alternate results location if desired, then start.
...
9. Open **Video library** to search transcripts and keywords, play videos, browse screenshots, and open exported files. Choosing a video slides the browser away, leaving a transcript-first workspace, compact source preview, and full-width screenshot gallery. Separate buttons open the generated results and original video's folder.
