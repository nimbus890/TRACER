...
def _read_version() -> str:
    """Read version from source or packaged application data."""
    here = Path(__file__).resolve().parent
    packaged = Path(getattr(sys, '_MEIPASS', here))
...
DEFAULTS = dict(model='tiny', interval=5.0, mode='interval', scene_threshold=0.22,
                ...
                skip_existing=True, output='', beta_features=False)
_dll_handles = []
...
def copy_file_with_progress(src, dst, on_bytes=None, cancel_check=None, chunk_size=4 * 1024 * 1024):
    """Copy file in chunks, invoking on_bytes(bytes_copied) and supporting cancellation."""
    src = Path(src)
    dst = Path(dst)
...
def export_collection(state, collection_id, destination, results=None, progress_callback=None, cancel_check=None):
    """Copy source video files from a collection into destination folder.
    Returns (copied_count, skipped_count, errors)."""
    results = state.get('results', []) if results is None else results
    ...
    for record in results:
        ...
        try:
            canonical = str(source.resolve()).casefold()
        except Exception:
    ...
    copied_bytes = 0
    total_files = len(valid_tasks)
    for source in valid_tasks:
        if cancel_check and cancel_check():
            break
        ...
        def on_chunk(b):
            ...
            if progress_callback and total_bytes > 0:
                ...
                progress_callback(pct, source.name, copied, total_files)
        ...
        try:
            copy_file_with_progress(source, target, on_bytes=on_chunk, cancel_check=cancel_check)
            copied += 1
        except OSError as e:
            errors.append((str(source), str(e)))

    if progress_callback and not (cancel_check and cancel_check()):
        progress_callback(100, 'Done', copied, total_files)
...
def _windows_memory_percentages(pid):
    ...
    app = 100.0 * float(counters.WorkingSetSize) / max(1, memory.ullTotalPhys)
    system = 100.0 * float(memory.ullTotalPhys - memory.ullAvailPhys) / max(1, memory.ullTotalPhys)
    return app, system


def _nvidia_usage_percentages(pid):
    ...
    system_gpu, used_vram, total_vram = [float(value.strip()) for value in first.split(',')[:3]]
    app_vram = 0.0
    ...
    if compute.returncode == 0:
        for line in compute.stdout.splitlines():
            ...
            if len(values) >= 2 and values[0].isdigit() and int(values[0]) == int(pid):
                try:
                    app_vram += float(values[1])
                except ValueError:
                    pass
    app_gpu = 0.0
    pmon = subprocess.run(['nvidia-smi', 'pmon', '-s', 'u', '-c', '1'],
                          capture_output=True, text=True, timeout=5, creationflags=flags)
    if pmon.returncode == 0:
        for line in pmon.stdout.splitlines():
            values = line.split()
            if len(values) >= 4 and values[1].isdigit() and int(values[1]) == int(pid):
                try:
                    app_gpu = max(app_gpu, float(values[3]))
                except ValueError:
                    pass
    return app_gpu, system_gpu, 100.0 * app_vram / max(1.0, total_vram), 100.0 * used_vram / max(1.0, total_vram)


def system_usage_snapshot(pid=None):
    """Return tiny app/system percentages without requiring telemetry or psutil."""
    pid = int(pid or os.getpid())
    result = {'gpu': (None, None), 'vram': (None, None), 'ram': (None, None)}
    try:
        app_gpu, system_gpu, app_vram, system_vram = _nvidia_usage_percentages(pid)
        result['gpu'] = (app_gpu, system_gpu)
        result['vram'] = (app_vram, system_vram)
    except (OSError, ValueError, subprocess.TimeoutExpired):
        pass
    if os.name == 'nt':
        try:
            result['ram'] = _windows_memory_percentages(pid)
        except OSError:
            pass
    return result
...
def validate_options(options):
    ...
    o['visual_interval'] = float(o['interval'])
...
def estimate(files, options):
    ...
    for v in selected:
        ...
        if o['transcribe']:
            size += v['duration'] * 100
        if o['visual_index']:
            visual_frames = max(2, math.ceil(v['duration'] / o['visual_interval']) + 1)
            size += visual_frames * 32_000
    return dict(count=count, visual_frames=sum(max(2, math.ceil(v['duration'] / o['visual_interval']) + 1)
                                               for v in selected) if o['visual_index'] else 0,
                bytes=int(size), duration=sum(v['duration'] for v in selected), videos=len(selected), approximate=True)
...
def estimate_runtime(duration, options):
    ...
    if rates:
        ...
        if wanted['visual_index']:
            rate += .015 * (30 / max(1, float(options.get('visual_interval', 30))))
...
def signature(path, options):
    ...
    raw = json.dumps([str(p.resolve()).casefold(), st.st_size, st.st_mtime_ns, relevant, 3], sort_keys=True)
    ...
    with av.open(str(path)) as container:
        ...
        for frame in container.decode(stream):
            ...
            if options['mode'] == 'interval':
                ...
                previous = small
            if save:
                if options.get('_timestamps_only'):
                    frames.append(dict(time=t))
                    last_saved = t
                    continue
...
class VisualIndexer:
    ...
    def index(self, path, out, duration, options, control, progress):
        ...
        interval = float(options.get('interval', 5.0))
        # Always inspect both boundaries; this catches opening/closing slates and
        # brief end shots without materially increasing storage.
        times = sorted({round(min(duration, n * interval), 4)
                        for n in range(max(1, math.ceil(duration / interval)))} |
                       {0.0, round(max(0.0, float(duration)), 4)})
        if options.get('mode') == 'scene':
            captures = extract_frames(path, out, {**options, '_timestamps_only': True}, control,
                                      lambda *_: None)
            times = sorted({float(frame['time']) for frame in captures} | {0.0, float(duration)})
        entries = []
        seen_actual = []
        ...
        for index, requested in enumerate(times):
            ...
            if not sample:
                continue
            image, actual = sample
            if any(abs(actual - previous) < .04 for previous in seen_actual):
            ...
            # Empty detections have nothing to contribute to object search. Do
            # not retain their thumbnails; the original footage remains intact.
            if not detections:
                progress(min(duration, requested + interval), len(entries))
                continue
            thumb = image.copy()
            thumb.thumbnail((320, 180), PILImage.Resampling.LANCZOS)
            relative = f'visual-index/{index + 1:06}_{timestamp(actual).replace(":", "-")}.jpg'
            thumb.save(out / relative, 'JPEG', quality=76, optimize=True)
        ...
        value = {'version': 2, 'interval': interval, 'samples_analyzed': len(times), 'keywords': summary,
...
def extract_detail_frames(source, output, start, duration, step=5.0):
    ...
    while current <= end + .01:
        ...
        current += step
    cached = sorted((path for path in folder.glob('*.jpg') if path.is_file()),
                    key=lambda path: path.stat().st_mtime, reverse=True)
    for stale in cached[180:]:
...
LOGS_DIR = DATA / 'logs'
LOG_FILE = LOGS_DIR / 'tracer.log'


def redact_path(path_str: str | Path) -> str:
    """Redact user home directory from paths to protect privacy."""
    try:
...
def log_diagnostic(message: str, category: str = 'INFO'):
    """Append a timestamped diagnostic event, redacting sensitive user paths and omitting transcript text."""
    try:
        LOGS_DIR.mkdir(parents=True, exist_ok=True)
        if LOG_FILE.exists() and LOG_FILE.stat().st_size > 2 * 1024 * 1024:
            old_log = LOGS_DIR / 'tracer.1.log'
            if old_log.exists():
                old_log.unlink()
...
def get_diagnostics_summary(state: dict = None) -> str:
    """Generate a clean, redacted diagnostics summary for bug reports."""
    lines = [
        f"Tracer Version: {APP_VERSION}",
        f"Platform: {sys.platform} ({ctypes.sizeof(ctypes.c_void_p) * 8}-bit)",
        f"Python: {sys.version.split()[0]}",
        f"Data Dir: {redact_path(DATA)}",
    ]
    if state:
        results = state.get('results', [])
        collections = state.get('collections', [])
        paper_edits = state.get('paper_edits', [])
        lines.append(f"Indexed Footage: {len(results)}")
        lines.append(f"Collections: {len(collections)}")
        lines.append(f"Paper Edits: {len(paper_edits)}")
        lines.append(f"Schema Version: {state.get('schema_version', 'legacy')}")
    return '\n'.join(lines)
