...
from PySide6.QtCore import Qt, QByteArray, QSize, QPoint, QPointF
from PySide6.QtGui import (QColor, QIcon, QPainter, QPen, QPixmap, QShortcut,
                           QKeySequence, QPolygon, QPainterPath)
...
def icon(name, color='#d8dfe3'):
    ...
    if name in ('landscape', 'portrait'):
    ...
    elif name in ('add', 'plus'):
        p.drawLine(10, 4, 10, 16)
        p.drawLine(4, 10, 16, 10)
    elif name in ('plus_timeline', 'insert_timeline'):
        # Plus sign on left
        p.drawLine(4, 5, 4, 13)
        p.drawLine(0, 9, 8, 9)
        # Upper track (Video)
        p.drawRoundedRect(9, 4, 4, 4, 1, 1)
        p.drawRoundedRect(14, 4, 5, 4, 1, 1)
        # Lower track (Audio)
        p.drawRoundedRect(9, 10, 6, 4, 1, 1)
        p.drawRoundedRect(16, 10, 3, 4, 1, 1)
        # Playhead needle
        p.setPen(QPen(QColor('#ff7a1a'), 1.1, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
        p.drawLine(14, 2, 14, 16)
    elif name == 'delete':
        p.drawLine(3, 5, 17, 5)
        p.drawLine(7, 2, 13, 2)
        ...
        path = QPainterPath()
        # Classic double-edge safety razor blade outline
        path.moveTo(4, 5)
        path.lineTo(16, 5)
        path.lineTo(18, 7)
        path.lineTo(18, 9)
        ...
        path.closeSubpath()
        p.drawPath(path)
        # Center slot mechanism
        p.drawEllipse(QPointF(10, 10), 2.0, 2.0)
        p.drawLine(5.5, 10, 8, 10)
        p.drawLine(12, 10, 14.5, 10)
        p.drawLine(6.5, 8.5, 6.5, 11.5)
        p.drawLine(13.5, 8.5, 13.5, 11.5)
    elif name == 'hand':
        p.drawLine(6, 9, 6, 5)
        p.drawLine(9, 9, 9, 3)
...
class WorkspaceBar(QWidget):
    def __init__(self, workspace, title, modes=()):
        ...
        layout = QToolButton()
        layout.setText('View')
        layout.setToolTip('View & layout options')
