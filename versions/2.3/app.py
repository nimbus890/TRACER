...
from PySide6.QtCore import (Qt, QThread, Signal, QTimer, QUrl, QSize, QPoint, QPointF, QRect, QRectF, QLockFile, QEvent,
...
from PySide6.QtGui import (QColor, QDesktopServices, QFont, QFontDatabase, QIcon, QPixmap, QImage,
                           ...
                           QPainterPath, QCursor, QRadialGradient)
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
    ...
    QGraphicsDropShadowEffect, QStyledItemDelegate, QStyle, QStyleOptionViewItem,
    QSizePolicy, QTabWidget, QGridLayout, QAbstractButton, QSizeGrip, QDialogButtonBox)
...
QDialog#processDialog { background: rgba(0, 0, 0, 126); }
QDialog#libraryOverlay { background: transparent; }
QFrame#processCard { background: #141414; border: 1px solid #2a2a2a; border-radius: 9px; }
QFrame#processCard QLabel, QFrame#processCard QCheckBox { background: transparent; border: 0; }
QFrame#processCard QLabel#processTitle { font-size: 20px; font-weight: 600; color: #ededed; }
QFrame#libraryCard { background: #141414; border: 1px solid #282828; border-radius: 9px; }
QFrame#libraryCard QLabel { background: transparent; }
...
QFrame#sidebar { background: transparent; border-right: 0; }
...
QWidget#panelHeader { background: #111111; border-bottom: 1px solid #1e1e1e; }
QLabel#panelTitle { background: transparent; font-size: 11px; font-weight: 600; letter-spacing: 0.5px; color: #b0b0b0; }
QDockWidget { border: 1px solid #1e1e1e; }
QMainWindow#modularWorkspace { background: #0c0c0c; }
QMainWindow::separator { width: 3px; height: 3px; background: #0c0c0c; }
QMainWindow::separator:hover { background: #ff7a1a; }
QFrame#toolsBoard { background: #141414; border: 1px solid #242424; border-radius: 6px; }
QToolButton#mediaViewSymbol { background: transparent; border: 1px solid #282828; border-radius: 4px; padding: 2px; }
QToolButton#mediaViewSymbol:hover { background: #1c1c1c; border-color: #3e3e3e; }
QToolButton#mediaViewSymbol:checked { background: #271a10; border-color: #ff7a1a; }
QSlider#videoScrubber::groove:horizontal { height: 3px; background: #262626; border-radius: 1px; }
QSlider#videoScrubber::sub-page:horizontal { height: 3px; background: #ff7a1a; border-radius: 1px; }
QSlider#videoScrubber::handle:horizontal { background: #ff7a1a; border: 2px solid #ff7a1a; width: 12px; height: 12px; margin: -5px 0; border-radius: 6px; }
QSlider#videoScrubber::handle:horizontal:hover { background: #ffa154; border-color: #ffa154; width: 14px; height: 14px; margin: -6px 0; border-radius: 7px; }
QSlider#volumeSlider::groove:vertical { width: 4px; background: #222222; border-radius: 2px; }
QSlider#volumeSlider::sub-page:vertical { width: 4px; background: #222222; border-radius: 2px; }
QSlider#volumeSlider::add-page:vertical { width: 4px; background: #555555; border-radius: 2px; }
QSlider#volumeSlider::handle:vertical { background: #888888; border: 0; width: 12px; height: 12px; margin: 0 -4px; border-radius: 6px; }
QSlider#volumeSlider::handle:vertical:hover { background: #b0b0b0; }
QSlider#volumeSlider::groove:horizontal { height: 3px; background: #222222; border-radius: 1px; }
QSlider#volumeSlider::sub-page:horizontal { height: 3px; background: #555555; border-radius: 1px; }
QSlider#volumeSlider::handle:horizontal { background: #888888; border: 0; width: 8px; height: 8px; margin: -3px 0; border-radius: 4px; }
QPushButton#ccBtn { background: transparent; border: 1px solid #333333; border-radius: 3px; font-size: 10px; font-weight: 700; color: #777777; padding: 0; }
QPushButton#ccBtn:hover { color: #ededed; border-color: #555555; }
QPushButton#ccBtn:checked { background: #2a1a0e; border-color: #ff7a1a; color: #ff7a1a; }
...
def tile_icon(kind, colour_override=None):
    ...
    if kind == 'audio':
        ...
        painter.drawLine(17, 5, 27, 5)
        painter.drawLine(20, 5, 20, 15)
        painter.drawLine(24, 5, 24, 15)
        painter.drawLine(20, 15, 12, 27)
        painter.drawLine(24, 15, 32, 27)
        painter.drawArc(12, 22, 20, 10, 180 * 16, 180 * 16)
        painter.drawLine(16, 22, 28, 22)
        painter.drawEllipse(19, 25, 2, 2)
        painter.drawEllipse(25, 24, 2, 2)
    elif kind == 'settings':
...
class ToggleSwitch(QAbstractButton):
    """Compact accessible switch used for global feature visibility."""
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setCheckable(True)
        self.setFixedSize(34, 18)
        self.setCursor(Qt.PointingHandCursor)
        self.setAccessibleName('Beta features')
        self.setToolTip('Beta')

    def paintEvent(self, event):
        painter = QPainter(self)
        painter.setRenderHint(QPainter.Antialiasing)
        track = QColor('#3f7f88' if self.isChecked() else '#30363a')
        if self.underMouse():
            track = track.lighter(118)
        painter.setPen(Qt.NoPen)
        painter.setBrush(track)
        painter.drawRoundedRect(0, 2, 34, 14, 7, 7)
        painter.setBrush(QColor('#f2f6f7' if self.isChecked() else '#9aa3a7'))
        painter.drawEllipse(18 if self.isChecked() else 2, 2, 14, 14)


class QueueAddButton(QPushButton):
    """Primary queue action with a quiet empty-state beacon."""
    def __init__(self, text, callback, parent=None):
        ...
        self.animation = QPropertyAnimation(self, b'beacon', self)
        self.animation.setDuration(1900)
        self.animation.setStartValue(0.18)
        self.animation.setEndValue(0.82)
    ...
    def set_beacon(self, value):
        self._beacon = float(value)
        alpha = round((22 + self._beacon * 62) if self._attention else (48 if self.underMouse() else 0))
        self.shadow.setColor(QColor(111, 224, 233, alpha))
        self.shadow.setBlurRadius(22 + self._beacon * 12)

    beacon = Property(float, get_beacon, set_beacon)

    def setAttention(self, enabled):
        enabled = bool(enabled and self.isEnabled())
        if enabled == self._attention:
            return
        self._attention = enabled
        if enabled:
    ...
    def enterEvent(self, event):
        super().enterEvent(event)
        self.set_beacon(max(self._beacon, .7))
...
class CursorAura(QWidget):
    """Atmospheric pointer halo strictly confined within its target surface."""
    def __init__(self, target, window=None, parent=None):
        if hasattr(target, 'sidebar') and window is not None:
            actual_target = window
            actual_window = target
        else:
            actual_target = target
            actual_window = window or (target.window() if hasattr(target, 'window') else None)
        super().__init__(actual_target)
        self.target = actual_target
        self.window = actual_window
        self.current = QPoint(-1000, -1000)
    ...
    def follow_cursor(self):
        if not self.target or not self.target.isVisible():
            ...
            return
        if self.geometry() != self.target.rect():
            self.setGeometry(self.target.rect())
        modal = QApplication.activeModalWidget()
        point = self.target.mapFromGlobal(QCursor.pos())
        is_dialog = isinstance(self.target.window(), QDialog)
        valid_modal = modal is None or (is_dialog and modal == self.target.window())
        visible = (self.window is None or self.window.isActiveWindow()) and self.target.rect().contains(point) and valid_modal
        self.setVisible(visible)
        if not visible:
            self.cursor_point = None
            return
        self.cursor_point = point
        if self.current.x() < -500:
            self.current = point
        else:
            self.current = QPoint(
                round(self.current.x() + (point.x() - self.current.x()) * .35),
                round(self.current.y() + (point.y() - self.current.y()) * .35),
            )
        self.raise_()
        self.update()
    ...
    def paintEvent(self, event):
        ...
        painter = QPainter(self)
        painter.setRenderHint(QPainter.Antialiasing)
        gradient = QRadialGradient(self.current, 150)
        gradient.setColorAt(0, QColor(95, 220, 229, 26))
        gradient.setColorAt(.45, QColor(95, 220, 229, 10))
        gradient.setColorAt(1, QColor(0, 0, 0, 0))
        painter.setPen(Qt.NoPen)
        painter.setBrush(QBrush(gradient))
        painter.drawEllipse(self.current, 150, 150)
...
class SidebarFrame(QFrame):
    """Left sidebar with a smooth, looping ambient gradient glow at the lower section."""
    def __init__(self, parent=None):
        super().__init__(parent)
        ...
        self._anim_timer.timeout.connect(self._on_glow_tick)
    ...
    def _on_glow_tick(self):
        self._glow_phase = (self._glow_phase + 0.025) % (2.0 * math.pi)
        if self.isVisible():
            self.update()
    ...
    def paintEvent(self, event):
        ...
        # Lower ambient looping glow
        glow_h = min(360, max(180, int(h * 0.46)))
        cy = float(h - int(glow_h * 0.40) + math.sin(self._glow_phase) * 18.0)
        cx = float((w * 0.5) + math.cos(self._glow_phase * 0.8) * 24.0)
        radius = float(170.0 + math.sin(self._glow_phase * 1.25) * 22.0)

        gradient = QRadialGradient(QPointF(cx, cy), radius)
        # Matches cursor aura palette: cyan-teal (#5fdce5) to deep oceanic blue to transparent
        gradient.setColorAt(0.0, QColor(95, 220, 229, 42))
        gradient.setColorAt(0.35, QColor(60, 175, 195, 22))
        gradient.setColorAt(0.70, QColor(25, 80, 130, 9))
        gradient.setColorAt(1.0, QColor(0, 0, 0, 0))

        painter.setPen(Qt.NoPen)
        painter.setBrush(QBrush(gradient))
        painter.drawRect(0, max(0, h - glow_h), w, glow_h)

        # Subtle 1px right border
        painter.setPen(QPen(QColor('#1e1e1e'), 1.0))
...
class MediaPickerDialog(QFileDialog):
    ...
    def __init__(self, parent=None):
        ...
        super().__init__(widget_parent, 'Add media', str(Path.home()), self.VIDEO_FILTER)
        self._media_paths = []
        ...
        self._add_folder_btn = QPushButton('Add this folder')
        self._add_folder_btn.clicked.connect(self._add_current_dir)
        btn_boxes = self.findChildren(QDialogButtonBox)
        if btn_boxes:
            btn_boxes[0].addButton(self._add_folder_btn, QDialogButtonBox.ActionRole)

    def _add_current_dir(self):
        curr = self.directory().absolutePath()
        if Path(curr).is_dir():
            self._media_paths = [curr]
            QDialog.accept(self)
            ...
            for le in self.findChildren(QLineEdit):
                ...
                if txt:
                    ...
                    if not p.is_absolute():
                        p = Path(self.directory().absolutePath()) / txt
                    if p.exists():
                        f, v = partition_media_paths([str(p)])
                        if f or v:
                            self._media_paths = f + v
                            break
        if not self._media_paths:
            curr = self.directory().absolutePath()
            if curr and Path(curr).is_dir() and Path(curr) != Path.home() and Path(curr).parent != Path(curr):
                self._media_paths = [curr]
                QDialog.accept(self)
                return
...
class MediaTableDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        if index.column() == 0:
            ...
            checked = state in (Qt.Checked, Qt.Checked.value, 2)
            hovered = bool(option.state & QStyle.State_MouseOver)
            if checked:
                border_color = QColor('#6c7684') if hovered else QColor('#505864')
                painter.setPen(QPen(border_color, 1.2))
                painter.setBrush(QBrush(QColor('#252b33')))
                painter.drawRoundedRect(box_rect, 2.5, 2.5)
                pen = QPen(QColor('#ffffff'), 1.8, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin)
                painter.setPen(pen)
                painter.drawLine(bx + 3.5, by + 8.5, bx + 7.0, by + 12.5)
                painter.drawLine(bx + 7.0, by + 12.5, bx + 13.5, by + 4.5)
            else:
                border_color = QColor('#525a66') if hovered else QColor('#3d434c')
                bg_color = QColor('#1a1e23') if hovered else QColor('#14171a')
                painter.setPen(QPen(border_color, 1.2))
            ...
            for video in folder['files']:
                ...
                def forward(kind, value):
                    if kind in stage_progress:
                        ...
                        percent_raw = (finished_weight + weight * current) / total_weight
                        percent = round(percent_raw * 100)
                        elapsed = time.time() - start_time
                        if percent_raw > 0.001:
                            eta_str = format_time_hms(max(0.0, (elapsed / percent_raw) - elapsed))
                        else:
                            eta_str = "Calculating..."
                        self.event.emit(f, p, 'overall', {'percent': percent, 'done': done_jobs, 'total': len(jobs), 'eta': eta_str})
                ...
                done_jobs += 1
                percent = round(finished_weight / total_weight * 100)
                elapsed = time.time() - start_time
                eta_str = format_time_hms(max(0.0, (elapsed / (finished_weight / total_weight)) - elapsed)) if finished_weight > 0 else "Calculating..."
                self.event.emit(f, p, 'overall', {'percent': percent, 'done': done_jobs, 'total': len(jobs), 'eta': eta_str})
...
class OptionsDialog(QDialog):
    def __init__(self, options, folders, parent, batch_name='folder batch'):
        ...
        self.closed_width = 760
        self.advanced_width = 1040
        self.beta_enabled = bool(getattr(parent, 'state', {}).get('settings', {}).get('beta_features', False))
        ...
        self.panel_scroll.setObjectName('processScroll')
        ...
        self.panel_scroll.setStyleSheet('QScrollArea#processScroll { background: transparent; border: 0; } QScrollArea#processScroll > QWidget { background: transparent; }')
        self.process_card = QFrame()
        self.process_card.setObjectName('processCard')
        self.process_card.setStyleSheet('QFrame#processCard { background: #141414; border: 1px solid #2a2a2a; border-radius: 9px; }')
        self.process_card.setMinimumWidth(self.closed_width - 2)
        self.panel_scroll.setWidget(self.process_card)
        self.panel_scroll.setFixedWidth(self.closed_width)
        self.setMouseTracking(True)
        self._cursor_point = None
        overlay.addWidget(self.panel_scroll, 0, Qt.AlignCenter)
        layout = QVBoxLayout(self.process_card)
        ...
        self.btn_dialog_new_project.setToolTip('Create a new collection')
        proj_row.addWidget(self.btn_dialog_new_project)

        self.project_title = label('Collection')
        ...
        self.visual_index.setVisible(self.beta_enabled)
        self.visual_reprocess_note.setVisible(self.beta_enabled)
    ...
    def toggle_advanced(self, checked):
        ...
        width = self.advanced_width if checked else self.closed_width
        self.process_card.setMinimumWidth(width - 2)
        self.panel_scroll.setFixedWidth(width)
    ...
    def showEvent(self, event):
        ...
        if parent:
            ...
            self.panel_scroll.setFixedHeight(min(620, max(460, parent.height() - 72)))

    def paintEvent(self, event):
        painter = QPainter(self)
        painter.setRenderHint(QPainter.Antialiasing)
        painter.fillRect(self.rect(), QColor(8, 10, 12, 195))
        if hasattr(self, '_cursor_point') and self._cursor_point is not None:
            card_geom = self.panel_scroll.geometry()
            if not card_geom.contains(self._cursor_point):
                gradient = QRadialGradient(self._cursor_point, 150)
                gradient.setColorAt(0, QColor(95, 220, 229, 24))
                gradient.setColorAt(.45, QColor(95, 220, 229, 9))
                gradient.setColorAt(1, QColor(0, 0, 0, 0))
                painter.setPen(Qt.NoPen)
                painter.setBrush(QBrush(gradient))
                painter.drawEllipse(self._cursor_point, 150, 150)

    def mouseMoveEvent(self, event):
        self._cursor_point = event.position().toPoint() if hasattr(event, 'position') else event.pos()
        self.update()
        super().mouseMoveEvent(event)

    def mousePressEvent(self, event):
        pos = event.position().toPoint() if hasattr(event, 'position') else event.pos()
        if not self.panel_scroll.geometry().contains(pos):
            self.reject()
            event.accept()
            return
        super().mousePressEvent(event)
    ...
    def eventFilter(self, watched, event):
        if event.type() == QEvent.MouseMove and self.isVisible():
            pos = event.globalPosition().toPoint() if hasattr(event, 'globalPosition') else (event.globalPos() if hasattr(event, 'globalPos') else None)
            if pos is not None:
                self._cursor_point = self.mapFromGlobal(pos)
                self.update()
        elif event.type() == QEvent.MouseButtonPress and self.isVisible():
    ...
    def dismiss_if_outside(self, global_position):
        ...
        if self.isVisible() and not self.panel_scroll.geometry().contains(local):
            self.reject()
            return True
        return False
    ...
    def values(self):
        ...
        vals = {**self.options, 'model': self.model.currentData(),
                ...
                'visual_index': self.beta_enabled and self.visual_index.isChecked(), 'visual_interval': self.interval.value(),
...
def format_time_hms(seconds_float):
    """Format seconds into HH:MM:SS matching Tracer video player spec."""
...
class VideoContainer(QWidget):
    """Container holding video display with an integrated floating caption overlay."""
    def __init__(self, content_widget=None, parent=None):
        ...
        self.caption_badge = QLabel(self.content_widget)
        self.caption_badge.setStyleSheet(
            'background: rgba(10, 10, 10, 0.88); color: #ffffff; font-size: 13px; font-weight: 500; '
            'padding: 6px 14px; border-radius: 4px; border: 1px solid rgba(255, 255, 255, 0.22);'
        )
        self.caption_badge.setAlignment(Qt.AlignCenter)
        self.caption_badge.setWordWrap(True)
        self.caption_badge.setAttribute(Qt.WA_TransparentForMouseEvents)
        self.caption_badge.hide()
        self.subtitle_badge = self.caption_badge

    def resizeEvent(self, event):
        super().resizeEvent(event)
        self._update_caption_position()

    def set_subtitles(self, text):
        self.set_caption_text(text)

    def set_caption_text(self, text):
        clean = (text or '').strip()
        if not clean:
            self.caption_badge.setText('')
            self.caption_badge.hide()
            return
        self.caption_badge.setText(clean)
        self.caption_badge.adjustSize()
        self._update_caption_position()
        self.caption_badge.show()
        self.caption_badge.raise_()

    def _update_caption_position(self):
        if not self.caption_badge.isVisible() and not self.caption_badge.text():
            return
        target = self.content_widget if self.content_widget else self
        w = min(target.width() - 32, max(120, self.caption_badge.sizeHint().width() + 28))
        h = max(28, self.caption_badge.sizeHint().height() + 8)
        x = (target.width() - w) // 2
        y = max(10, target.height() - h - 18)
        self.caption_badge.setGeometry(x, y, w, h)
        self.caption_badge.raise_()


class VolumeFlyout(QFrame):
    """Vertical volume slider popup flyout anchored above the volume button."""
    def __init__(self, parent=None):
        super().__init__(parent, Qt.Popup | Qt.FramelessWindowHint)
        self.setObjectName('volumeFlyout')
        self.setStyleSheet(
            'QFrame#volumeFlyout { background: #141414; border: 1px solid #282828; border-radius: 6px; }'
        )
        layout = QVBoxLayout(self)
        layout.setContentsMargins(6, 10, 6, 10)
        layout.setSpacing(6)
        self.setFixedSize(34, 115)
...
class VideoControlBar(QWidget):
    """Unified media player control bar matching Tracer 2.1 design."""
    playClicked = Signal()
    seekMoved = Signal(int)
    captionsToggled = Signal(bool)
    ...
    def __init__(self, parent=None):
        ...
        self.setStyleSheet('QWidget#videoControlBar { background: #111111; border-top: 1px solid #1e1e1e; min-height: 36px; }')
        ...
        self.play_btn.setIcon(tile_icon('play'))
        self.play_btn.setIconSize(QSize(18, 18))
        self.play_btn.setFixedSize(30, 26)
        self.play_btn.setStyleSheet('QPushButton { background: transparent; border: 0; } QPushButton:hover { background: #222222; border-radius: 4px; }')
        self.play_btn.clicked.connect(self.playClicked.emit)
        self.play_btn.setToolTip('Play / Pause')
        layout.addWidget(self.play_btn)

        # 2. Timecode: 00:00:00 / 00:00:00
        self.time_label = QLabel('00:00:00 / 00:00:00')
        self.time_label.setStyleSheet('font-family: "Cascadia Mono", Consolas, monospace; font-size: 11px; color: #878787;')
        layout.addWidget(self.time_label)

        # 3. Scrubber slider
        self.seek = MarkedSlider(Qt.Horizontal)
        self.seek.setObjectName('videoScrubber')
        self.seek.sliderMoved.connect(self.seekMoved.emit)
        layout.addWidget(self.seek, 1)

        # 4. Captions [cc] button
        self.cc_btn = QPushButton('CC')
        self.cc_btn.setCheckable(True)
        self.cc_btn.setObjectName('ccBtn')
        self.cc_btn.setFixedSize(30, 22)
        self.cc_btn.setToolTip('Toggle Captions / Subtitles')
        self.cc_btn.toggled.connect(self.captionsToggled.emit)
        layout.addWidget(self.cc_btn)

        # 5. Volume button with vertical flyout
        self.vol_btn = QPushButton()
        self.vol_btn.setIcon(workspace.icon('volume'))
        self.vol_btn.setIconSize(QSize(18, 18))
        self.vol_btn.setFixedSize(26, 26)
        self.vol_btn.setStyleSheet('QPushButton { background: transparent; border: 0; } QPushButton:hover { background: #222222; border-radius: 4px; }')
        self.vol_btn.setToolTip('Adjust Volume')
        self.vol_btn.clicked.connect(self.on_volume_button_clicked)
        layout.addWidget(self.vol_btn)

        self.vol_flyout = VolumeFlyout(self)
        self.vol_slider = self.vol_flyout.slider
        self.vol_slider.valueChanged.connect(self._on_volume_changed)
        self._last_volume = 80

        # 6. Fullscreen button
        self.fs_btn = QPushButton()
        self.fs_btn.setIcon(workspace.icon('fullscreen'))
        self.fs_btn.setIconSize(QSize(18, 18))
        self.fs_btn.setFixedSize(26, 26)
        self.fs_btn.setStyleSheet('QPushButton { background: transparent; border: 0; } QPushButton:hover { background: #222222; border-radius: 4px; }')
        self.fs_btn.setToolTip('Toggle Fullscreen')
        self.fs_btn.clicked.connect(self.fullscreenClicked.emit)
        layout.addWidget(self.fs_btn)

        # Aliases for convenience
        self.scrubber = self.seek
        self.mute_btn = self.vol_btn
        self.volume_slider = self.vol_slider
        self.fullscreen_btn = self.fs_btn
    ...
    @property
    def is_muted(self):
        return self.vol_slider.value() == 0

    @property
    def captions_enabled(self):
        return self.cc_btn.isChecked()

    @captions_enabled.setter
    def captions_enabled(self, val):
        self.cc_btn.setChecked(bool(val))

    def on_volume_button_clicked(self):
        self.toggle_volume_flyout()

    def toggle_volume_flyout(self):
        if self.vol_flyout.isVisible():
            self.vol_flyout.hide()
        else:
            pt = self.vol_btn.mapToGlobal(QPoint(0, 0))
            x = pt.x() + (self.vol_btn.width() - self.vol_flyout.width()) // 2
            y = pt.y() - self.vol_flyout.height() - 4
            self.vol_flyout.move(x, y)
            self.vol_flyout.show()
            self.vol_flyout.raise_()
    ...
    def toggle_mute(self):
        if self.vol_slider.value() > 0:
            ...
            self.vol_slider.setValue(self._last_volume or 80)

    def _on_volume_changed(self, value):
        self.vol_btn.setIcon(workspace.icon('mute' if value == 0 else 'volume'))
        self.volumeChanged.emit(value / 100.0)

    def set_playing(self, playing):
        self.play_btn.setIcon(tile_icon('pause' if playing else 'play'))
        self.play_btn.setToolTip('Pause' if playing else 'Play')

    def set_duration(self, seconds):
        self._duration_sec = max(0.0, float(seconds))
        self.seek.setRange(0, round(self._duration_sec * 1000))
        self._update_clock()

    def set_position(self, seconds):
        self._position_sec = max(0.0, float(seconds))
        if not self.seek.isSliderDown():
            self.seek.blockSignals(True)
            self.seek.setValue(round(self._position_sec * 1000))
            self.seek.blockSignals(False)
        self._update_clock()

    def _update_clock(self):
        cur_str = format_time_hms(self._position_sec)
        dur_str = format_time_hms(self._duration_sec)
...
class ResultsPage(QWidget):
    def __init__(self, parent=None):
        ...
        self.paper_collection_button = button('Open in Paper Edit', lambda: self.open_collection_editor('paper'))
        collection_actions.addWidget(self.paper_collection_button)
        collection_actions.addWidget(button('Open in Timeline', lambda: self.open_collection_editor('story')))
        ...
        self.add_to_btn = button('Add To ▾', self.on_add_to_collection, primary=True)
        self.add_to_btn.setEnabled(False)
        self.add_to_btn.setObjectName('addToCollectionBtn')
        self.add_to_btn.setStyleSheet(
            'QPushButton#addToCollectionBtn { background: #ededed; color: #0c0c0c; border: 1px solid #ededed; '
            'border-radius: 6px; padding: 7px 16px; font-weight: 600; font-size: 12px; } '
            'QPushButton#addToCollectionBtn:hover { background: #ffffff; } '
            'QPushButton#addToCollectionBtn:disabled { background: #222222; color: #555555; border-color: #222222; }'
        )
        self.add_to_collection_btn = self.add_to_btn
        action_bar.addWidget(self.add_to_btn)
        ...
        self.preview_splitter.setStyleSheet('QSplitter::handle { background: #202020; margin: 1px 10px; border-radius: 2px; } QSplitter::handle:hover { background: #3a3a3a; }')
        video_panel = QWidget()
        preview_layout = QVBoxLayout(video_panel)
        ...
        self.preview_project_combo.setToolTip('Choose a collection for this video')
        ...
        self.btn_new_proj.setToolTip('Create a new collection and add this video')
        ...
        self.btn_add_to_proj.setToolTip('Add this video to the selected collection')
        ...
        self.video.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Expanding)
        self.player = QMediaPlayer(self)
        self.audio = QAudioOutput(self)
        ...
        self.video_container = VideoContainer(self.video)
        player_layout.addWidget(self.video_container, 1)

        self.control_bar = VideoControlBar(self)
        self.control_bar.playClicked.connect(self.play)
        self.control_bar.seekMoved.connect(self.player.setPosition)
        self.control_bar.volumeChanged.connect(self.audio.setVolume)
        self.control_bar.fullscreenClicked.connect(self.toggle_fullscreen)
        self.control_bar.captionsToggled.connect(self.on_captions_toggled)

        # Legacy compatibility aliases
        self.play_button = self.control_bar.play_btn
        self.seek = self.control_bar.seek
        self.clock = self.control_bar.time_label

        self.player.positionChanged.connect(self.position)
        self.player.durationChanged.connect(lambda n: self.control_bar.set_duration(n / 1000) if n > 0 else None)
        self.player.playbackStateChanged.connect(self.sync_play_icon)
        player_layout.addWidget(self.control_bar)
        preview_layout.addWidget(player_frame, 1)
        self.preview_splitter.addWidget(video_panel)
        ...
        self.visual_tab_index = self.tabs.indexOf(visual_tab)
        ...
        evidence_layout.addWidget(self.tabs, 1)
        self.preview_splitter.addWidget(evidence_panel)
        saved_split = getattr(self.window, 'state', {}).get('library_preview_split', [390, 360])
        self.preview_splitter.setSizes(saved_split if isinstance(saved_split, list) and len(saved_split) == 2 else [390, 360])
        self.preview_splitter.splitterMoved.connect(self.remember_preview_split)
        ...
        self.set_beta_enabled(self.beta_enabled, refresh=False)

    def remember_preview_split(self, *args):
        if self.window and hasattr(self.window, 'state'):
            self.window.state['library_preview_split'] = self.preview_splitter.sizes()
            self.window.save_timer.start(250)

    def configure_scope_filter(self):
        current = self.scope_filter.currentData()
        ...
        if self.beta_enabled:
            ...
            self.scope_filter.addItem('Transcripts only', 'audio')
        index = self.scope_filter.findData(current)
        self.scope_filter.setCurrentIndex(index if index >= 0 else 0)
        self.scope_filter.blockSignals(False)

    def set_beta_enabled(self, enabled, refresh=True):
        self.beta_enabled = bool(enabled)
        if hasattr(self, 'scope_filter'):
            self.configure_scope_filter()
        if hasattr(self, 'visual_tab_index'):
            self.tabs.setTabVisible(self.visual_tab_index, self.beta_enabled)
            if not self.beta_enabled and self.tabs.currentIndex() == self.visual_tab_index:
                self.tabs.setCurrentIndex(0)
        if hasattr(self, 'paper_collection_button'):
            self.paper_collection_button.setVisible(self.beta_enabled)
    ...
    def on_add_media(self):
        if self.active_collection_id != 'all':
            picker = LibraryPickerDialog(self.window.state.get('results', []), getattr(self, 'window', self))
    ...
    def on_export_project(self):
        ...
        if not records:
            QMessageBox.information(self, 'Empty collection', 'There are no videos in this collection to export.')
            return
        coll = next((c for c in self.window.state.get('collections', []) if c['id'] == self.active_collection_id), None)
        name = coll.get('name', 'Collection') if coll else 'Collection'
        self.export_files(None, records, title=f"Export '{name}'")
        return
    ...
    def populate(self):
        ...
        for record in reversed(self.records):
            ...
            if not self.beta_enabled:
                origins = [origin for origin in origins if origin != 'visual']
    ...
    def on_table_item_clicked(self, item, column):
        if column == 0:
            last_item = getattr(self, '_last_toggled_item', None)
            last_time = getattr(self, '_last_toggled_time', 0)
            if last_item is item and (time.time() - last_time) < 0.25:
                return  # Handled by Qt indicator
            if item:
    ...
    def on_add_to_collection(self):
        if not self.checked_ids or not self.window:
            return
        dialog = AddToCollectionDialog(self.window.state, list(self.checked_ids), self)
    ...
    def on_overflow_menu(self):
        ...
        if action == a_select_all:
            ...
            self.export_files(None, [r for r in self.records if r.get('id') in self.checked_ids], title='Export Selected Videos')
    ...
    def export_files(self, destination, records, title='Export project'):
        if not records:
            return
        snapshot = copy.deepcopy(records)
        dlg = ExportProjectDialog(
            parent=self,
            export_fn=lambda dest, prog, cancel: core.export_collection(
                self.window.state, getattr(self, 'active_collection_id', 'all'),
                dest, results=snapshot, progress_callback=prog, cancel_check=cancel
            ),
            title=title,
            subtitle=f"Export {len(snapshot)} video{'s' if len(snapshot) != 1 else ''} to destination folder.",
            default_destination=destination
        )
        dlg.exec()

    def open_collection_editor(self, kind):
        if kind == 'paper' and not self.beta_enabled:
            return
        records = release121.collection_records(self.window.state, self.active_collection_id)
        if self.checked_ids:
    ...
    def _get_record_segments(self):
        if not self.record:
            return []
        segments = self.record.get('segments')
        if segments:
            return segments
        output_dir = self.record.get('output')
        if output_dir:
            ...
            if json_path.exists():
                try:
                    ...
                    if isinstance(loaded, list):
                        self.record['segments'] = loaded
                        return loaded
                except Exception:
                    pass
        src = self.record.get('source')
        if src:
            side_json = Path(src).with_suffix('.json')
            if side_json.exists():
                try:
                    loaded = json.loads(side_json.read_text(encoding='utf-8'))
                    if isinstance(loaded, list):
                        self.record['segments'] = loaded
                        return loaded
                    elif isinstance(loaded, dict) and 'segments' in loaded:
                        self.record['segments'] = loaded['segments']
    ...
    def position(self, n):
        self.control_bar.set_position(n / 1000)
        if hasattr(self, 'video_container') and self.control_bar.cc_btn.isChecked() and self.record:
            sec = n / 1000.0
            segments = self._get_record_segments()
            seg = next((s for s in segments if float(s.get('start', 0)) <= sec <= float(s.get('end', 0)) + 0.15), None)
            self.video_container.set_caption_text(seg.get('text', '') if seg else '')
        elif hasattr(self, 'video_container'):
            self.video_container.set_caption_text('')

    def on_captions_toggled(self, enabled):
        if not enabled and hasattr(self, 'video_container'):
            self.video_container.set_caption_text('')
        elif hasattr(self, 'video_container'):
...
class CardResizeGrip(QSizeGrip):
    """Custom size grip that resizes a modal card inside an overlay."""
    def __init__(self, target_card, parent=None):
        super().__init__(parent or target_card)
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            self._start_size = self.target_card.size()
            event.accept()

    def mouseMoveEvent(self, event):
        if self._dragging:
            cur_pos = event.globalPosition().toPoint() if hasattr(event, 'globalPosition') else event.globalPos()
            delta = cur_pos - self._start_pos
            overlay = self.target_card.window()
            max_w = (overlay.width() - self.target_card.x() - 15) if overlay else 2000
            max_h = (overlay.height() - self.target_card.y() - 15) if overlay else 1500
            new_w = max(760, min(max_w, self._start_size.width() + delta.x()))
            new_h = max(500, min(max_h, self._start_size.height() + delta.y()))
            self.target_card.resize(new_w, new_h)
            event.accept()
...
class ResizableCard(QFrame):
    """Modal card container that can be dragged around by its header and resized from any of its 8 edges/corners."""
    BORDER_MARGIN = 8
    MIN_WIDTH = 760
    MIN_HEIGHT = 500
    ...
    def _hit_edges(self, pos):
        edges = set()
        if pos.x() <= self.BORDER_MARGIN:
            edges.add('left')
        elif pos.x() >= self.width() - self.BORDER_MARGIN:
            edges.add('right')
        if pos.y() <= self.BORDER_MARGIN:
            edges.add('top')
        elif pos.y() >= self.height() - self.BORDER_MARGIN:
            edges.add('bottom')
        return edges
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            if pos.y() <= 60 and (child is None or isinstance(child, (QLabel, QFrame))):
    ...
    def mouseMoveEvent(self, event):
        ...
        pos = event.position().toPoint() if hasattr(event, 'position') else event.pos()
        if self._resizing and self._drag_start_geo is not None:
            delta = gpos - self._drag_start_pos
            geo = QRect(self._drag_start_geo)
            parent = self.parentWidget()
            max_w = (parent.width() - 20) if parent else 2000
            max_h = (parent.height() - 20) if parent else 1500

            if 'right' in self._resize_edges:
                geo.setWidth(max(self.MIN_WIDTH, min(max_w - geo.x(), self._drag_start_geo.width() + delta.x())))
            elif 'left' in self._resize_edges:
                max_left_shift = self._drag_start_geo.width() - self.MIN_WIDTH
                shift = max(-self._drag_start_geo.x() + 10, min(max_left_shift, delta.x()))
                geo.setLeft(self._drag_start_geo.left() + shift)

            if 'bottom' in self._resize_edges:
                geo.setHeight(max(self.MIN_HEIGHT, min(max_h - geo.y(), self._drag_start_geo.height() + delta.y())))
            elif 'top' in self._resize_edges:
                max_top_shift = self._drag_start_geo.height() - self.MIN_HEIGHT
                shift = max(-self._drag_start_geo.y() + 10, min(max_top_shift, delta.y()))
                geo.setTop(self._drag_start_geo.top() + shift)

            self.setGeometry(geo)
            event.accept()
            return
        elif self._dragging_header and self._drag_start_geo is not None:
            delta = gpos - self._drag_start_pos
            parent = self.parentWidget()
            max_x = (parent.width() - 80) if parent else 2000
            max_y = (parent.height() - 80) if parent else 1500
            new_x = max(10, min(max_x, self._drag_start_geo.x() + delta.x()))
            new_y = max(10, min(max_y, self._drag_start_geo.y() + delta.y()))
    ...
    def mouseReleaseEvent(self, event):
        ...
        self._drag_start_geo = None
        pos = event.position().toPoint() if hasattr(event, 'position') else event.pos()
        self._update_cursor_for_edges(self._hit_edges(pos))
        super().mouseReleaseEvent(event)


class ExportWorker(QThread):
    progress = Signal(int, str, int, int)  # percent, filename, done_count, total_count
    succeeded = Signal(object)             # result object / target path
    failed = Signal(str)                  # error string

    def __init__(self, export_fn, destination, parent=None):
        super().__init__(parent)
    ...
    def run(self):
        try:
            def prog_cb(pct, name, done, total):
                if not self._cancelled:
                    self.progress.emit(pct, name, done, total)

            result = self.export_fn(self.destination, prog_cb, self.is_cancelled)
            if self._cancelled:
                return
...
class ExportProjectDialog(QDialog):
    """Overlay dialog for exporting projects or collections with live percentage progress,
    backdrop blur, cursor aura, and click-outside collapse."""
    def __init__(self, parent=None, export_fn=None, title='Export Project', subtitle='Export media and project files to destination folder.', default_destination=None, on_complete=None):
        super().__init__(parent)
        self.export_fn = export_fn
        ...
        win = getattr(parent, 'window', None)
        if callable(win):
            try:
                win = win()
            except Exception:
                win = None
        if not win and isinstance(parent, QWidget):
            win = parent.window()
        self.top_window = win
        self.background_target = win.centralWidget() if (win and hasattr(win, 'centralWidget') and callable(getattr(win, 'centralWidget', None))) else None
        self.background_blur = None
        if self.background_target and hasattr(self.background_target, 'graphicsEffect') and self.background_target.graphicsEffect() is None:
            self.background_blur = QGraphicsBlurEffect(self.background_target)
            self.background_blur.setBlurRadius(3.5)
            self.background_target.setGraphicsEffect(self.background_blur)
        ...
        if not default_destination:
            ...
            if user_videos.exists():
                default_destination = str(user_videos)
            else:
                default_destination = str(Path.home() / 'Documents')
        self.default_destination = default_destination

        self.card = QFrame(self)
        self.card.setObjectName('processCard')
        self.card.setStyleSheet('QFrame#processCard { background: #141414; border: 1px solid #2a2a2a; border-radius: 9px; }')
        ...
        header = QVBoxLayout()
        header.setSpacing(3)
        header.addWidget(label('EXPORT PROJECT', 'eyebrow'))
        self.title_label = label(self.title_text, 'title')
        self.title_label.setStyleSheet('font-size: 20px; font-weight: 600; color: #ededed;')
        header.addWidget(self.title_label)
        self.subtitle_label = label(self.subtitle_text, 'subtle')
        header.addWidget(self.subtitle_label)
        layout.addLayout(header)

        dest_box = QVBoxLayout()
        dest_box.setSpacing(6)
        dest_box.addWidget(label('Destination folder', 'subtle'))
        dest_row = QHBoxLayout()
        dest_row.setSpacing(8)
        self.dest_edit = QLineEdit(self.default_destination)
        self.dest_edit.setPlaceholderText('Choose destination folder…')
        dest_row.addWidget(self.dest_edit, 1)
        self.browse_btn = button('Browse…', self.browse_destination)
        ...
        self.progress_bar.setStyleSheet(
            'QProgressBar { background: #141414; color: #ededed; border: 1px solid #282828; '
            'border-radius: 6px; min-height: 22px; text-align: center; font-size: 11px; font-weight: 600; } '
            'QProgressBar::chunk { background: #2563eb; border-radius: 5px; }'
        )
        self.progress_bar.setVisible(False)
        prog_box.addWidget(self.progress_bar)

        self.status_label = label('Ready to export.', 'subtle')
        self.status_label.setStyleSheet('color: #878787; font-size: 12px;')
        prog_box.addWidget(self.status_label)
        layout.addLayout(prog_box)

        layout.addStretch()

        actions = QHBoxLayout()
        self.open_folder_btn = button('Open in Explorer', self.open_exported_folder)
        self.open_folder_btn.setVisible(False)
        actions.addWidget(self.open_folder_btn)

        actions.addStretch()

        self.cancel_btn = button('Cancel', self.reject)
        actions.addWidget(self.cancel_btn)

        self.export_btn = button('Export', self.start_export, primary=True)
    ...
    def browse_destination(self):
        chosen = QFileDialog.getExistingDirectory(self, 'Select Destination Folder', self.dest_edit.text() or str(Path.home()))
        if chosen:
    ...
    def start_export(self):
        ...
        try:
            ...
            QMessageBox.warning(self, 'Invalid folder', f'Could not create destination folder:\n{exc}')
            return
        ...
        self.worker = ExportWorker(self.export_fn, dest, self)
        self.worker.progress.connect(self.on_progress)
        self.worker.succeeded.connect(self.on_succeeded)
        self.worker.failed.connect(self.on_failed)
        self.worker.start()

    def on_progress(self, percent, current_file, done_count, total_count):
        self.progress_bar.setValue(percent)
        if total_count > 0:
            self.status_label.setText(f"{current_file} ({done_count}/{total_count}) · {percent}%")
        else:
            self.status_label.setText(f"{current_file} · {percent}%")

    def on_succeeded(self, result):
        self.progress_bar.setValue(100)
        self.export_btn.setText('Done')
        self.export_btn.setEnabled(False)
        self.cancel_btn.setText('Close')
        self.result_target = result

        if isinstance(result, tuple) and len(result) >= 3:
            copied, skipped, errors = result[0], result[1], result[2]
            msg = f"Done · Exported {copied} video{'s' if copied != 1 else ''}"
            if skipped:
                msg += f", {skipped} skipped"
            if errors:
                msg += f", {len(errors)} failed"
            self.status_label.setText(msg)
            self.result_target = self.dest_edit.text().strip()
        else:
            self.status_label.setText(f"Done · Export completed to {Path(str(result)).name}")
    ...
    def on_failed(self, error_msg):
        ...
        self.export_btn.setText('Retry')

    def open_exported_folder(self):
        target = self.result_target or self.dest_edit.text().strip()
        if target:
            p = Path(target)
    ...
    def showEvent(self, event):
        super().showEvent(event)
        parent_win = self.top_window or (self.parentWidget().window() if self.parentWidget() else (self.parent().window() if hasattr(self.parent(), 'window') else None))
        if parent_win and parent_win != self:
            tl = parent_win.mapToGlobal(QPoint(0, 0))
            self.setGeometry(tl.x(), tl.y(), parent_win.width(), parent_win.height())
        else:
            self.setGeometry(0, 0, 1400, 900)

        card_w = min(620, max(500, int(self.width() * 0.52)))
        card_h = min(360, max(300, int(self.height() * 0.42)))
        cx = max(10, (self.width() - card_w) // 2)
        cy = max(10, (self.height() - card_h) // 2)
        self.card.setGeometry(cx, cy, card_w, card_h)

    def resizeEvent(self, event):
        super().resizeEvent(event)
        if hasattr(self, 'card') and self.card.width() > 0:
            card_w = min(620, max(500, int(self.width() * 0.52)))
            card_h = min(360, max(300, int(self.height() * 0.42)))
            cx = max(10, (self.width() - card_w) // 2)
            cy = max(10, (self.height() - card_h) // 2)
    ...
    def paintEvent(self, event):
        painter = QPainter(self)
        painter.setRenderHint(QPainter.Antialiasing)
        painter.fillRect(self.rect(), QColor(5, 7, 9, 218))
        if hasattr(self, '_cursor_point') and self._cursor_point is not None:
            if hasattr(self, 'card') and not self.card.geometry().contains(self._cursor_point):
                painter.save()
                path = QPainterPath()
                path.addRect(QRectF(self.rect()))
                card_rect = QRectF(self.card.geometry()).adjusted(-1, -1, 1, 1)
                card_path = QPainterPath()
                card_path.addRoundedRect(card_rect, 9, 9)
                path = path.subtracted(card_path)
                painter.setClipPath(path)

                gradient = QRadialGradient(QPointF(self._cursor_point), 420)
                gradient.setColorAt(0.0, QColor(95, 220, 229, 44))
                gradient.setColorAt(0.35, QColor(95, 220, 229, 20))
                gradient.setColorAt(0.70, QColor(40, 130, 180, 8))
                gradient.setColorAt(1.0, QColor(0, 0, 0, 0))
                painter.setPen(Qt.NoPen)
                painter.setBrush(QBrush(gradient))
    ...
    def mousePressEvent(self, event):
        pos = event.position().toPoint() if hasattr(event, 'position') else event.pos()
        if hasattr(self, 'card') and not self.card.geometry().contains(pos):
    ...
    def eventFilter(self, watched, event):
        if event.type() == QEvent.MouseMove and self.isVisible():
            ...
            if pos is not None:
                ...
                if hasattr(self, 'card') and not self.card.geometry().contains(local):
                    self.close_or_cancel()
                    return True
        return super().eventFilter(watched, event)
...
class LibraryPickerDialog(QDialog):
    ...
    def __init__(self, records, parent=None):
        ...
        self.checked_ids = set()
        self.setObjectName('libraryOverlay')
        self.setWindowFlags(Qt.Dialog | Qt.FramelessWindowHint)
        self.setAttribute(Qt.WA_TranslucentBackground)
        ...
        self.card = ResizableCard(self)
        self.card_frame = self.card
        ...
        if callable(win):
            try:
                ...
                win = None
        beta = False
        if win and hasattr(win, 'state') and isinstance(getattr(win, 'state', None), dict):
            beta = win.state.get('settings', {}).get('beta_features', False)
        elif parent and hasattr(parent, 'state') and isinstance(getattr(parent, 'state', None), dict):
            beta = parent.state.get('settings', {}).get('beta_features', False)
        subtitle = ('Search transcript text, filenames, and indexed visual terms. Select as many videos as you need.'
                    if beta else 'Search transcript text and filenames. Select as many videos as you need.')
        words.addWidget(label(subtitle, 'subtle'))
        ...
        self.sizegrip = CardResizeGrip(self.card, self.card)
        actions.addWidget(self.sizegrip, 0, Qt.AlignBottom | Qt.AlignRight)
        layout.addLayout(actions)
        self.refresh()
        app_inst = QApplication.instance()
        if app_inst:
    ...
    def showEvent(self, event):
        ...
        card_w = max(960, min(1440, int(self.width() * 0.88)))
        card_h = max(620, min(980, int(self.height() * 0.85)))
        cx = max(10, (self.width() - card_w) // 2)
        cy = max(10, (self.height() - card_h) // 2)
        self.card.setGeometry(cx, cy, card_w, card_h)
    ...
    def resizeEvent(self, event):
        super().resizeEvent(event)
        if hasattr(self, 'card') and self.card.width() > 0:
            cur_w = min(self.card.width(), max(760, self.width() - 20))
            cur_h = min(self.card.height(), max(500, self.height() - 20))
            cur_x = max(10, min(self.width() - cur_w - 10, self.card.x()))
            cur_y = max(10, min(self.height() - cur_h - 10, self.card.y()))
            self.card.setGeometry(cur_x, cur_y, cur_w, cur_h)
...
class AddToCollectionDialog(QDialog):
    """Frameless overlay with backdrop blur, cursor radial glow, and sidebar-styled collection list."""
    def __init__(self, state, video_ids, parent=None):
        super().__init__(parent)
        self.state = state
        self.video_ids = list(video_ids)
        self.selected_collection_id = None
        self.setObjectName('addToCollectionOverlay')
        self.setWindowFlags(Qt.Dialog | Qt.FramelessWindowHint)
        self.setAttribute(Qt.WA_TranslucentBackground)
        self.setWindowTitle('Add to Collection')

        # Blur parent central widget if available
        win = parent.window if hasattr(parent, 'window') else None
        if callable(win):
            try:
                win = win()
            except Exception:
                win = None
        self.background_target = win.centralWidget() if (win and hasattr(win, 'centralWidget') and callable(getattr(win, 'centralWidget', None))) else (parent.centralWidget() if (parent and hasattr(parent, 'centralWidget') and callable(getattr(parent, 'centralWidget', None))) else None)
        self.background_blur = None
        if self.background_target and hasattr(self.background_target, 'graphicsEffect') and self.background_target.graphicsEffect() is None:
            self.background_blur = QGraphicsBlurEffect(self.background_target)
            self.background_blur.setBlurRadius(3.5)
        ...
        self.card = QFrame()
        self.card.setObjectName('addToCard')
        self.card.setFixedWidth(440)
        self.card.setStyleSheet(
            'QFrame#addToCard { background: #141619; border: 1px solid #2a2d32; border-radius: 10px; }'
        )
        ...
        header = QVBoxLayout()
        header.setSpacing(3)
        header.addWidget(label('COLLECTIONS', 'eyebrow'))
        title = label('Add To Collection', 'title')
        title.setStyleSheet('font-size: 16px; font-weight: 600; color: #ffffff;')
        header.addWidget(title)
        vcount = len(self.video_ids)
        sub_text = f"Adding {vcount} selected video{'s' if vcount != 1 else ''}"
        sub = label(sub_text, 'subtle')
        sub.setStyleSheet('color: #8e949e; font-size: 12px;')
        header.addWidget(sub)
        card_layout.addLayout(header)

        # Collections list styled like the sidebar
        self.coll_list = QListWidget()
        self.coll_list.setObjectName('addToCollList')
        self.coll_list.setIconSize(QSize(16, 16))
        self.coll_list.setFixedHeight(230)
        self.coll_list.setStyleSheet(
            'QListWidget#addToCollList { '
            '  background: #101214; border: 1px solid #23272d; border-radius: 7px; outline: none; padding: 4px; '
            '} '
            'QListWidget#addToCollList::item { '
            '  padding: 8px 10px; border-radius: 5px; color: #d0d4dc; font-size: 13px; margin: 2px 0px; '
            '} '
            'QListWidget#addToCollList::item:hover { '
            '  background: #1a1e24; color: #ffffff; '
            '} '
            'QListWidget#addToCollList::item:selected { '
            '  background: #232932; color: #5fdce5; font-weight: 500; '
            '}'
        )
        self.populate_collections()
        card_layout.addWidget(self.coll_list)

        # New collection input row
        new_row = QHBoxLayout()
        new_row.setSpacing(8)
        self.new_input = QLineEdit()
        self.new_input.setPlaceholderText('Create new collection…')
        self.new_input.setStyleSheet(
            'QLineEdit { background: #0e1012; border: 1px solid #262a30; border-radius: 6px; '
            'padding: 6px 10px; color: #ededed; font-size: 12px; } '
            'QLineEdit:focus { border: 1px solid #5fdce5; }'
        )
        self.new_input.returnPressed.connect(self.create_and_select)
        new_row.addWidget(self.new_input, 1)
        self.create_btn = button('Create', self.create_and_select)
        self.create_btn.setFixedHeight(30)
        new_row.addWidget(self.create_btn)
        card_layout.addLayout(new_row)
        ...
        self.coll_list.itemDoubleClicked.connect(self.on_item_double_clicked)
    ...
    def showEvent(self, event):
        ...
        if self.parentWidget():
            parent_win = self.parentWidget().window()
        elif self.parent():
    ...
    def paintEvent(self, event):
        painter = QPainter(self)
        painter.setRenderHint(QPainter.Antialiasing)
        painter.fillRect(self.rect(), QColor(6, 8, 10, 215))
        if hasattr(self, '_cursor_point') and self._cursor_point is not None:
            if hasattr(self, 'card') and not self.card.geometry().contains(self._cursor_point):
                gradient = QRadialGradient(self._cursor_point, 200)
                gradient.setColorAt(0, QColor(95, 220, 229, 32))
                gradient.setColorAt(.4, QColor(95, 220, 229, 12))
                gradient.setColorAt(1, QColor(0, 0, 0, 0))
                painter.setPen(Qt.NoPen)
                painter.setBrush(QBrush(gradient))
                painter.drawEllipse(self._cursor_point, 200, 200)
    ...
    def populate_collections(self):
        ...
        # 1. First item: + Create New Collection
        new_item = QListWidgetItem('+ Create New Collection')
        new_item.setIcon(QIcon(tile_icon('collection')))
        new_item.setData(Qt.UserRole, '__new__')
        new_item.setForeground(QColor('#5fdce5'))
        self.coll_list.addItem(new_item)

        # 2. Existing collections sorted by latest modified first
        existing = [c for c in self.state.get('collections', []) if c.get('id') != 'all']
        def sort_key(col):
            t = col.get('updated_at') or col.get('created_at') or col.get('timestamp')
            if t is not None:
                try:
                    return float(t)
        ...
        if any(sort_key(c) > 0 for c in existing):
        ...
        for c in existing:
            ...
            item.setIcon(QIcon(tile_icon(icon_name)))
    ...
    def selection_changed(self):
        ...
        if cid == '__new__':
            ...
            self.selected_collection_id = cid
    ...
    def create_and_select(self):
        ...
        if not name:
            name, ok = QInputDialog.getText(self, 'Create New Collection', 'Collection name:')
    ...
    def apply_add(self):
        if self.selected_collection_id and self.selected_collection_id != '__new__':
            release121.migrate(self.state)
            records_to_add = [r for r in self.state.get('results', []) if r.get('id') in self.video_ids]
            release121.attach(self.state, self.selected_collection_id, records_to_add)
            for c in self.state.get('collections', []):
                if c.get('id') == self.selected_collection_id:
                    c['updated_at'] = time.time()
                    break
            self.accept()
...
CollectionPickerDialog = AddToCollectionDialog
...
class PaperEditPage(QWidget):
    def __init__(self, window):
        ...
        self._auto_compact_panels = set()
        self._pending_sync_ids = set()
        self._sync_timer = QTimer(self)
        self._sync_timer.setSingleShot(True)
        self._sync_timer.setInterval(350)
        self._sync_timer.timeout.connect(self.sync_pending_documents)
        ...
        self.paper_scroll.setAlignment(Qt.AlignHCenter | Qt.AlignTop)
        self.paper_scroll.setStyleSheet('QScrollArea { background: #101010; border: 0; }')
        ...
        self.desk.add_panel('script', '', centre)
        ...
        self.sequence.itemChanged.connect(self.passage_inclusion_changed)
        order = QWidget()
        ol = QVBoxLayout(order)
        ol.setContentsMargins(8, 8, 8, 8)
        ol.addWidget(label('Check passages to include · drag to change story order', 'subtle'))
        ...
        self.workspace_bar.row.addWidget(button('Open in Timeline', self.open_storyline))
    ...
    def resizeEvent(self, event):
        """Protect the manuscript width without changing the saved desktop layout."""
        super().resizeEvent(event)
        if not hasattr(self, 'desk'):
            return
        compact = self.width() < 1080
        if compact:
            for key in ('preview', 'frames'):
                dock = self.desk.panels.get(key)
                if dock and dock.isVisible():
                    self._auto_compact_panels.add(key)
                    dock.hide()
        elif self._auto_compact_panels:
            for key in tuple(self._auto_compact_panels):
                dock = self.desk.panels.get(key)
                if dock:
                    dock.show()
            self._auto_compact_panels.clear()

    def open_storyline(self):
        if self.document:
            collection, sequence = self.sync_document_now(self.document)
            self.window.projects_page.refresh(collection['id'])
    ...
    def document_changed(self):
        ...
        self.update_sequence()
        if self.document:
            self._pending_sync_ids.add(self.document['id'])
            self.paper.status_line.setText('Updating…')
            self._sync_timer.start()
        self.window.save_timer.start(250)

    def sync_document_now(self, document):
        """Synchronize one manuscript without losing queued edits from another."""
        self._pending_sync_ids.discard(document['id'])
        collection, sequence = release121.sync_paper(self.window.state, document)
        if self.document and self.document.get('id') == document.get('id'):
            self.paper.status_line.setText(paper_sync.status_badge_text(document))
        self.window.save_timer.start(100)
        return collection, sequence

    def sync_pending_documents(self):
        pending = set(self._pending_sync_ids)
        self._pending_sync_ids.clear()
        for document in self.window.state.get('paper_edits', []):
            if document.get('id') in pending:
                self.sync_document_now(document)
    ...
    def update_sequence(self):
        ...
        self.sequence.clear()
        if self.document:
            for segment in paper_edit.ordered_segments(self.document, included_only=False):
                ...
                item.setFlags(item.flags() | Qt.ItemIsUserCheckable)
                item.setCheckState(Qt.Checked if segment.get('included', True) else Qt.Unchecked)
                if not segment.get('included', True):
                    item.setForeground(QColor('#777777'))
                self.sequence.addItem(item)
        self.sequence.blockSignals(False)

    def passage_inclusion_changed(self, item):
        if not self.document or not item:
            return
        segment_id = item.data(Qt.UserRole)
        segment = next((value for value in self.document.get('segments', [])
                        if value.get('id') == segment_id), None)
        if not segment:
            return
        included = item.checkState() == Qt.Checked
        if segment.get('included', True) == included:
            return
        segment['included'] = included
        self.paper.set_document(self.document)
        self.update_analysis()
        self.sync_document_now(self.document)

    def sequence_reordered(self, *args):
        if not self.document:
            return
        ordered = [self.sequence.item(index).data(Qt.UserRole) for index in range(self.sequence.count())]
        missing = [value['id'] for value in self.document.get('segments', []) if value['id'] not in ordered]
        self.document['order'] = ordered + missing
        self.paper.set_document(self.document)
        self.sync_document_now(self.document)
        self.window.save_timer.start(150)
    ...
    def export_script(self):
        ...
        if chosen == premiere:
            ...
            dlg = ExportProjectDialog(
                parent=self,
                export_fn=lambda dest, prog, cancel: release121.export_paper_project(
                    snapshot, dest, progress_callback=prog, cancel_check=cancel
                ),
                title="Export Paper Edit Project",
                subtitle="Collect paper sequence media, script, and project files.",
                on_complete=self.export_finished
            )
            dlg.exec()
        return
...
class LegacyProjectsPage(QWidget):
    ...
    def refresh(self, select_id=None):
        ...
        if select_id:
            self.source_collection_id = select_id
    ...
    def set_busy(self, busy):
        for widget in (self.projects, self.add_folders, self.add_files, self.remove_source):
    ...
    def create_new_sequence(self):
        ...
        if not project:
            return
        seq_num = len(project.setdefault('sequences', [])) + 1
        seq = timeline.new_sequence(f'Sequence {seq_num}')
        project['sequences'].append(seq)
        self.populate_sequences(seq['id'])
        self.window.save_timer.start(200)

    def delete_current_sequence(self):
        index = self.sequence_tabs.currentIndex()
        if index >= 0:
            self.close_sequence(index)

    def close_sequence(self, index):
        project = self.current()
        if not project or len(project['sequences']) <= 1:
            self.status.setText('Every collection keeps at least one sequence. Create another before closing this one.')
            return
    ...
    def sequence_command(self, command):
        ...
        if command in ('cut', 'delete', 'duplicate', 'split', 'ripple_delete') and not clip:
            self.status.setText('Select a timeline clip first.')
            self.timeline_canvas.setFocus()
            return
    ...
    def insert_selected_media(self):
        payloads = []
        is_grid = hasattr(self, 'media_stack') and self.media_stack.currentWidget() == getattr(self, 'media_grid', None)
        if is_grid and hasattr(self, 'media_grid'):
            selected = self.media_grid.selectedItems()
            if selected:
                payloads = [item.data(Qt.UserRole) for item in selected if item and item.data(Qt.UserRole)]
            elif self.media_grid.currentItem():
                p = self.media_grid.currentItem().data(Qt.UserRole)
                if p:
                    payloads = [p]
        else:
            selected = self.tree.selectedItems()
            if selected:
                payloads = [item.data(0, Qt.UserRole + 1) for item in selected if item and item.data(0, Qt.UserRole + 1)]
            elif self.tree.currentItem():
                p = self.tree.currentItem().data(0, Qt.UserRole + 1)
                if p:
                    payloads = [p]
        if not payloads:
            # If no file is selected, import all media from this collection
            if is_grid and hasattr(self, 'media_grid'):
                for i in range(self.media_grid.count()):
                    item = self.media_grid.item(i)
                    if item and item.data(Qt.UserRole):
                        payloads.append(item.data(Qt.UserRole))
            else:
                for i in range(self.tree.topLevelItemCount()):
                    item = self.tree.topLevelItem(i)
                    if item and not item.isDisabled():
                        ...
                        if p:
                            payloads.append(p)
        if payloads:
            self.add_sequential_media_to_timeline(payloads)
        else:
            self.status.setText('No media available to insert.')

    def add_sequential_media_to_timeline(self, payloads, start=None, track_index=None):
        sequence = self.active_sequence()
        if not sequence or not payloads:
            return
        ...
        audio_target = next((value for value in tracks
                             if value.get('type') == 'audio' and value.get('name') == 'A1' and not value.get('locked')), None)
        if audio_target is None:
            audio_target = next((value for value in tracks
                                 if value.get('type') == 'audio' and not value.get('locked')), None)
        ...
        for video in payloads:
            ...
            linked = uuid.uuid4().hex if video.get('audio') and audio_target is not None else None
            clip = timeline.make_clip(video['path'], duration, cursor,
                                      name=video.get('name'), linked=linked)
            target['clips'].append(clip)
            last_clip = clip
            if linked and audio_target is not None:
                audio_target['clips'].append(timeline.make_clip(
                    video['path'], duration, cursor,
                    name=video.get('name'), linked=linked))
            cursor = round(cursor + duration, 4)

        if last_clip:
            self.timeline_canvas.selected_id = last_clip['id']
            if payloads and isinstance(payloads[-1], dict):
                self.inspect_source(payloads[-1])
        collection_id = getattr(self, 'source_collection_id', None)
        if collection_id:
            self.timeline_canvas.set_media_records(release121.collection_records(self.window.state, collection_id) or [])
        self.finish_sequence_edit()
        self.status.setText(f'Inserted {len(payloads)} clip{"s" if len(payloads) != 1 else ""} sequentially.')

    def add_media_to_timeline(self, video, start=None, track_index=None):
        if isinstance(video, (list, tuple)):
            self.add_sequential_media_to_timeline(video, start=start, track_index=track_index)
            return
        sequence = self.active_sequence()
    ...
    def collect_media(self):
        ...
        if timeline.sequence_duration(sequence) <= 0:
            ...
            return
        missing = timeline.offline_sources(sequence)
        seq_name = sequence.get('name', 'Sequence')

        def on_collected(target):
            if missing:
        ...
        dlg = ExportProjectDialog(
            parent=self,
            export_fn=lambda dest, prog, cancel: timeline.collect_sequence_media(
                sequence, dest, progress_callback=prog, cancel_check=cancel
            ),
            title="Export Timeline as Project",
            subtitle=f"Collect media and sequence project for '{seq_name}'.",
            on_complete=on_collected
        )
        dlg.exec()
...
class ProjectsPage(LegacyProjectsPage):
    ...
    def __init__(self, window):
        ...
        self.page_title = label('Timeline', 'title')
    ...
    def install_modular_workspace(self):
        ...
        self.desk.configure('timeline', {
            'Story timeline': [(['preview', 'timeline'], 800), (['media'], 360)],
            'Landscape': [(['preview', 'timeline'], 800), (['media'], 360)],
            'Portrait': [(['preview'], 300), (['media', 'timeline'], 1000)],
            'Timeline focus': [(['media'], 240), (['timeline'], 1050)],
        })
        self.workspace_bar = workspace.WorkspaceBar(self.desk, 'Timeline')
        self.landscape_button = workspace.tool('landscape', 'Horizontal timeline layout', lambda: self.set_story_orientation('Landscape'))
        self.portrait_button = workspace.tool('portrait', 'Vertical timeline layout', lambda: self.set_story_orientation('Portrait'))
    ...
    def set_story_orientation(self, name):
        ...
        if name in ('Landscape', 'Story timeline'):
    ...
    def build_empty_editor(self):
        ...
        self.paper_import_button = button('← Import from Paper Edit', lambda: self.window.navigate(2))
        self.paper_import_button.setToolTip('Go back to Paper Edit, select passages, then click → Timeline to send them here')
        path_row_2.addWidget(self.paper_import_button)
        path_row_2.addStretch()
    ...
    def empty_project_action(self):
        self.collection_selector.setFocus()
        self.collection_selector.showPopup()

    def set_beta_enabled(self, enabled):
        if hasattr(self, 'paper_import_button'):
            self.paper_import_button.setVisible(bool(enabled))
    ...
    def build_media_panel(self):
        ...
        self.collection_selector.setToolTip('Choose the collection shown in Sequence Media')
        self.collection_selector.setAccessibleName('Select Collection')
        self.collection_selector.currentIndexChanged.connect(self.source_collection_changed)
        self.add_media_button = self.collection_selector
        self.remove_source = button('−', self.window.remove_project_source)
        self.remove_source.setToolTip('Remove selected source from this collection')
        self.remove_source.hide()
        source_actions.addWidget(self.add_media_button, 1)

        self.insert_media_button = button('', self.insert_selected_media)
        self.insert_media_button.setIcon(workspace.icon('plus_timeline'))
        self.insert_media_button.setIconSize(QSize(18, 18))
        self.insert_media_button.setToolTip('Insert media into timeline sequentially (or selected files)')
        self.insert_media_button.setFixedSize(28, 28)
        self.insert_media_button.setStyleSheet(
            'QPushButton { background: transparent; border: 1px solid transparent; border-radius: 4px; padding: 2px; } '
            'QPushButton:hover { background: #222222; border-color: #3e3e3e; } '
            'QPushButton:pressed { background: #2a2a2a; border-color: #555555; }'
        )
        source_actions.addWidget(self.insert_media_button)

        self.relink_media_btn = button('Relink', self.relink_offline_media)
        self.relink_media_btn.setObjectName('relinkMediaBtn')
        self.relink_media_btn.setIcon(workspace.icon('relink'))
        self.relink_media_btn.setIconSize(QSize(14, 14))
        self.relink_media_btn.setToolTip('Relink missing or offline sequence media')
        self.relink_media_btn.setStyleSheet(
            'QPushButton#relinkMediaBtn { background: #141414; border: 1px solid #282828; border-radius: 4px; '
            'padding: 4px 6px; font-size: 11px; color: #878787; font-weight: 500; } '
            'QPushButton#relinkMediaBtn:hover { background: #1e1e1e; border-color: #444; color: #ededed; }'
        )
        self.relink_button = self.relink_media_btn
        source_actions.addWidget(self.relink_media_btn)
        ll.addLayout(source_actions)

        # Search row with List / Grid symbols
        search_row = QHBoxLayout()
        ...
        search_row.addWidget(media_search, 1)

        self.list_mode_btn = QToolButton()
        self.list_mode_btn.setObjectName('mediaViewSymbol')
        self.list_mode_btn.setIcon(workspace.icon('list'))
        self.list_mode_btn.setIconSize(QSize(16, 16))
        self.list_mode_btn.setFixedSize(26, 26)
        self.list_mode_btn.setCheckable(True)
        self.list_mode_btn.setToolTip('List view')

        self.grid_mode_btn = QToolButton()
        self.grid_mode_btn.setObjectName('mediaViewSymbol')
        self.grid_mode_btn.setIcon(workspace.icon('grid'))
        self.grid_mode_btn.setIconSize(QSize(16, 16))
        self.grid_mode_btn.setFixedSize(26, 26)
        self.grid_mode_btn.setCheckable(True)
        self.grid_mode_btn.setToolTip('Grid view')

        self.view_mode_group = QButtonGroup(self)
        self.view_mode_group.setExclusive(True)
        self.view_mode_group.addButton(self.list_mode_btn)
        self.view_mode_group.addButton(self.grid_mode_btn)

        is_grid = bool(getattr(self.window, 'state', {}).get('settings', {}).get('sequence_media_grid', False))
        self.list_mode_btn.toggled.connect(lambda c: self.set_media_symbol_mode(False) if c else None)
        self.grid_mode_btn.toggled.connect(lambda c: self.set_media_symbol_mode(True) if c else None)
        (self.grid_mode_btn if is_grid else self.list_mode_btn).setChecked(True)

        def _proxy_set_checked(val):
            (self.grid_mode_btn if val else self.list_mode_btn).setChecked(True)
            self.set_media_symbol_mode(bool(val))

        self.media_view_toggle = type('ToggleProxy', (), {
            'isChecked': lambda s: self.grid_mode_btn.isChecked(),
            'setChecked': lambda s, v: _proxy_set_checked(v),
            'toggled': self.grid_mode_btn.toggled
        })()

        search_row.addWidget(self.list_mode_btn)
        search_row.addWidget(self.grid_mode_btn)
        ...
        self.tree.setIconSize(QSize(72, 42))
        ...
        self.media_grid = timeline.MediaBinList()
        self.media_grid.setViewMode(QListWidget.IconMode)
        self.media_grid.setIconSize(QSize(96, 56))
        self.media_grid.setGridSize(QSize(108, 86))
        self.media_grid.setResizeMode(QListWidget.Adjust)
        self.media_grid.setWordWrap(True)
        self.media_grid.setDragEnabled(True)
        self.media_grid.setSelectionMode(QAbstractItemView.ExtendedSelection)
        self.media_grid.setStyleSheet('QListWidget { background: #0c0c0c; border: 0; } QListWidget::item { color: #878787; font-size: 10px; padding: 4px; } QListWidget::item:selected { background: #1a1a1a; color: #ffffff; }')
        self.media_grid.itemDoubleClicked.connect(
            lambda item: self.add_media_to_timeline(item.data(Qt.UserRole)) if item else None)
        self.media_grid.currentItemChanged.connect(self.grid_item_changed)
        self.media_stack = QStackedWidget()
        self.media_stack.addWidget(self.tree)
        self.media_stack.addWidget(self.media_grid)
        ll.addWidget(self.media_stack, 1)

        if is_grid:
            self.media_stack.setCurrentWidget(self.media_grid)
        self.media_hint = label('Drag or double-click to insert', 'subtle')
        self.media_hint.setStyleSheet('color: #555555; font-size: 9px;')
        ...
        self.status = label('Select a collection, then drag footage onto the timeline.', 'subtle')
        self.status.setWordWrap(True)
    ...
    def filter_media(self, query):
        ...
        for i in range(self.tree.topLevelItemCount()):
            ...
            if parent.childCount() == 0:
                parent.setHidden(bool(query and query not in parent.text(0).casefold()))
                continue
            visible = False
            ...
            if query:
                parent.setExpanded(True)
        if hasattr(self, 'media_grid'):
            for i in range(self.media_grid.count()):
                item = self.media_grid.item(i)
                item.setHidden(bool(query and query not in (item.text() or '').casefold()))
    ...
    def on_clip_trimmed(self, clip_id):
        sequence = self.active_sequence()
        document = paper_sync.linked_document(self.window.state, sequence)
        if not sequence or not document:
            return
        document_id = document['id']
        if paper_sync.sync_sequence_trim_to_paper(sequence, document, clip_id):
            paper_page = getattr(self.window, 'paper_edit_page', None)
            if paper_page and paper_page.document and paper_page.document.get('id') == document_id:
                paper_page.paper.update_active_passage_badge()
                paper_page.paper.status_line.setText(paper_sync.status_badge_text(document))
                paper_page.update_analysis()
            self.window.save_timer.start(250)
    ...
    def on_timeline_command_triggered(self, cmd):
        if cmd == 'play_pause':
            ...
            self.fit_timeline()
        elif cmd in ('undo', 'redo', 'copy', 'paste', 'cut'):
    ...
    def build_editing_panel(self):
        ...
        self.sequence_container = VideoContainer(self.preview_stack)
        source_layout.addWidget(self.sequence_container, 1)

        self.sequence_control_bar = VideoControlBar(self)
        self.sequence_control_bar.playClicked.connect(self.toggle_sequence_playback)
        self.sequence_control_bar.seekMoved.connect(lambda value: self.timeline_canvas.set_playhead(value / 1000, notify=True))
        self.sequence_control_bar.volumeChanged.connect(self.audio.setVolume)
        self.sequence_control_bar.captionsToggled.connect(self.on_sequence_captions_toggled)
        self.sequence_control_bar.fullscreenClicked.connect(self.toggle_sequence_fullscreen)

        # Compatibility aliases
        self.play_button = self.sequence_control_bar.play_btn
        self.source_seek = self.sequence_control_bar.seek
        self.source_clock = self.sequence_control_bar.time_label
        self.sequence_play = self.sequence_control_bar.play_btn
        self.sequence_video_container = self.sequence_container
        self.sequence_preview_bar = self.sequence_control_bar
        source_layout.addWidget(self.sequence_control_bar)
        ...
        self.new_sequence_btn = button('', self.create_new_sequence)
        self.new_sequence_btn.setIcon(workspace.icon('plus'))
        self.new_sequence_btn.setIconSize(QSize(14, 14))
        self.new_sequence_btn.setFixedSize(26, 26)
        self.new_sequence_btn.setToolTip('New Sequence (+)')
        sequence_header.addWidget(self.new_sequence_btn)

        self.duplicate_sequence_btn = button('', self.duplicate_sequence)
        self.duplicate_sequence_btn.setIcon(workspace.icon('duplicate'))
        self.duplicate_sequence_btn.setIconSize(QSize(14, 14))
        self.duplicate_sequence_btn.setFixedSize(26, 26)
        self.duplicate_sequence_btn.setToolTip('Duplicate Sequence')
        sequence_header.addWidget(self.duplicate_sequence_btn)

        self.delete_sequence_btn = button('', self.delete_current_sequence)
        self.delete_sequence_btn.setIcon(workspace.icon('delete'))
        self.delete_sequence_btn.setIconSize(QSize(14, 14))
        self.delete_sequence_btn.setFixedSize(26, 26)
        self.delete_sequence_btn.setToolTip('Delete Sequence')
        sequence_header.addWidget(self.delete_sequence_btn)
        ...
        toolbar_frame.setObjectName('timelineToolbar')
        toolbar_layout = QHBoxLayout(toolbar_frame)
        toolbar_layout.setContentsMargins(0, 0, 0, 0)
        toolbar_layout.setSpacing(10)

        tools_board = QFrame()
        tools_board.setObjectName('toolsBoard')
        self.tools_board = tools_board
        tool_grid = QGridLayout(tools_board)
        tool_grid.setContentsMargins(3, 2, 3, 2)
        tool_grid.setHorizontalSpacing(3)
        tool_grid.setVerticalSpacing(2)

        self.edit_buttons = {}
        self.tool_buttons = {}

        # Row 0: 6 pointer tools + Undo, Redo, Split (9 tools)
        top_column = 0
        for name, tip in (
            ('select', 'Selection Tool (V) · Select and move clips'),
            ('track_select', 'Track Select Forward (A) · Select all clips to the right (Shift for all tracks)'),
            ('ripple', 'Ripple Edit (B) · Trim edge and ripple downstream edits'),
            ('razor', 'Razor Tool (C) · Split clip at pointer'),
            ('hand', 'Hand Tool (H) · Pan timeline view'),
            ('zoom', 'Zoom Tool (Z) · Click to zoom in, Alt+click to zoom out'),
        ):
            control = button('', lambda checked=False, n=name: self.select_timeline_tool(n))
            control.setCheckable(True)
            control.setFixedSize(30, 26)
            control.setIcon(workspace.icon(name))
            control.setIconSize(QSize(16, 16))
            control.setToolTip(tip)
            control.setAccessibleName(tip.split(' · ', 1)[0])
            control.setObjectName('timelineTool')
            control.setChecked(name == 'select')
            self.tool_buttons[name] = control
            tool_grid.addWidget(control, 0, top_column)
            top_column += 1
        ...
        for text, command_name in (('Undo', 'undo'), ('Redo', 'redo'), ('Split', 'split')):
            ...
            action.setFixedSize(30, 26)
            action.setAccessibleName(explanation)
            self.edit_buttons[command_name] = action
            tool_grid.addWidget(action, 0, top_column)
            top_column += 1

        # Row 1: Copy, Paste, Duplicate, Delete + Track Actions (Solo, Add Video, Add Audio, Marker) + Snap (9 tools)
        bottom_column = 0
        for text, command_name in (('Copy', 'copy'), ('Paste', 'paste'),
            ...
            action.setAccessibleName(explanation)
            self.edit_buttons[command_name] = action
            tool_grid.addWidget(action, 1, bottom_column)
            bottom_column += 1

        # Track actions without toggle and lock (already present on track headers)
        track_actions = (
            ('video_track', 'Add video track', lambda: self.add_track('video')),
            ('audio_track', 'Add audio track', lambda: self.add_track('audio')),
        ...
        for icon_name, tip, callback in track_actions:
            action = button('', callback)
            action.setObjectName('timelineTool')
            action.setIcon(workspace.icon(icon_name))
            action.setIconSize(QSize(16, 16))
            action.setFixedSize(30, 26)
            action.setToolTip(tip)
            action.setAccessibleName(tip)
            tool_grid.addWidget(action, 1, bottom_column)
            bottom_column += 1

        self.snap = button('')
        self.snap.setObjectName('timelineTool')
        self.snap.setIcon(workspace.icon('snap'))
        self.snap.setIconSize(QSize(16, 16))
        self.snap.setFixedSize(30, 26)
        ...
        self.snap.setAccessibleName('Snap clips and playhead')
        self.snap.toggled.connect(lambda enabled: setattr(self.timeline_canvas, 'snap_enabled', enabled))
        tool_grid.addWidget(self.snap, 1, bottom_column)

        toolbar_layout.addWidget(tools_board)
        toolbar_layout.addStretch()

        fit = button('', self.fit_timeline)
        fit.setObjectName('timelineTool')
        fit.setIcon(workspace.icon('fit'))
        fit.setIconSize(QSize(18, 18))
        fit.setFixedSize(30, 26)
        fit.setToolTip('Fit the complete sequence in the timeline view · \\')
        fit.setAccessibleName('Fit complete sequence')

        self.zoom = MarkedSlider(Qt.Horizontal)
        self.zoom.setRange(2, 80)
        self.zoom.setValue(10)
        self.zoom.setMinimumWidth(110)
        self.zoom.setMaximumWidth(150)
        ...
        self.zoom_out = button('−', lambda: self.zoom.setValue(max(self.zoom.minimum(), self.zoom.value() - 6)))
        self.zoom_out.setFixedSize(22, 22)
        self.zoom_out.setStyleSheet('QPushButton { background: #161616; border: 1px solid #282828; border-radius: 4px; color: #878787; font-size: 14px; font-weight: bold; padding: 0; } QPushButton:hover { color: #ededed; border-color: #444; }')
        self.zoom_out.setToolTip('Zoom out timeline (−)')

        self.zoom_in = button('+', lambda: self.zoom.setValue(min(self.zoom.maximum(), self.zoom.value() + 6)))
        self.zoom_in.setFixedSize(22, 22)
        self.zoom_in.setStyleSheet('QPushButton { background: #161616; border: 1px solid #282828; border-radius: 4px; color: #878787; font-size: 14px; font-weight: bold; padding: 0; } QPushButton:hover { color: #ededed; border-color: #444; }')
        self.zoom_in.setToolTip('Zoom in timeline (+)')
        self.zoom_out_btn = self.zoom_out
        self.zoom_in_btn = self.zoom_in

        right_controls = QHBoxLayout()
        right_controls.setContentsMargins(0, 0, 0, 0)
        right_controls.setSpacing(5)
        right_controls.addWidget(self.zoom_out)
        right_controls.addWidget(self.zoom)
        right_controls.addWidget(self.zoom_in)
        right_controls.addWidget(fit)
        toolbar_layout.addLayout(right_controls)

        cl.addWidget(toolbar_frame)
    ...
    def toggle_sequence_playback(self):
        ...
        if self.sequence_playing:
            ...
            return
        dur = timeline.sequence_duration(sequence)
        if self.timeline_canvas.playhead >= dur:
        ...
        if hasattr(self, 'sequence_control_bar'):
            self.sequence_control_bar.set_playing(True)
            self.sequence_control_bar.set_duration(dur)
    ...
    def stop_sequence_playback(self):
        ...
        self.player.pause()
        if hasattr(self, 'sequence_control_bar'):
            self.sequence_control_bar.set_playing(False)
        self.store_sequence_view()

    def update_sequence_caption(self, position):
        if not hasattr(self, 'sequence_control_bar') or not self.sequence_control_bar.cc_btn.isChecked():
            if hasattr(self, 'sequence_container'):
                self.sequence_container.set_caption_text('')
            return
        sequence = self.active_sequence()
        ...
        if not clip:
            if hasattr(self, 'sequence_container'):
                self.sequence_container.set_caption_text('')
            return
        source_path = str(clip.get('source', ''))
        record = next((r for r in self.window.state.get('results', [])
                       if str(r.get('source', '')).casefold() == source_path.casefold()), None)
        if not record:
            if hasattr(self, 'sequence_container'):
                self.sequence_container.set_caption_text('')
            return
        clip_time = float(clip.get('source_in', 0)) + float(position) - float(clip.get('start', 0))
        seg = next((s for s in record.get('segments', [])
                    if float(s.get('start', 0)) <= clip_time <= float(s.get('end', 0))), None)
        if hasattr(self, 'sequence_container'):
            self.sequence_container.set_caption_text(seg.get('text', '') if seg else '')

    def on_sequence_captions_toggled(self, enabled):
        if not enabled and hasattr(self, 'sequence_container'):
            self.sequence_container.set_caption_text('')
        else:
            self.update_sequence_caption(self.timeline_canvas.playhead)

    def toggle_sequence_fullscreen(self):
        top = self.window()
        if top:
            if top.isFullScreen():
                top.showNormal()
    ...
    def timeline_playhead_changed(self, position):
        dur = timeline.sequence_duration(self.active_sequence() or {})
        if hasattr(self, 'sequence_control_bar'):
            self.sequence_control_bar.set_duration(dur)
            self.sequence_control_bar.set_position(position)
        else:
            self.source_seek.setRange(0, round(dur * 1000))
            self.source_seek.setValue(round(position * 1000))
        if self.sequence_playing:
            self.stop_sequence_playback()
        self.seek_sequence_preview(position)
        self.update_sequence_caption(position)
        self.store_sequence_view()
    ...
    def build_export_panel(self):
        ...
        # Relink media relocated to Sequence Media panel
    ...
    def refresh(self, select_id=None):
        ...
        for project in self.window.state['projects']:
            ...
            self.projects.addItem(item)
        self.projects.blockSignals(False)
        self.refresh_collection_selector(current)
        if current:
            self.select_project(current)
        elif self.projects.count():
            self.projects.setCurrentRow(0)
    ...
    def refresh_collection_selector(self, preferred=None):
        if not hasattr(self, 'collection_selector'):
            return
        release121.migrate(self.window.state)
        current = preferred or getattr(self, 'source_collection_id', None) or self.window.state.get('storyline_source_collection') or 'all'
        self.collection_selector.blockSignals(True)
        self.collection_selector.clear()
        self.collection_selector.addItem('Select Collection', None)
        for collection in self.window.state.get('collections', []):
            self.collection_selector.addItem(collection.get('name', 'Untitled'), collection.get('id'))
        index = self.collection_selector.findData(current)
        self.collection_selector.setCurrentIndex(index if index >= 0 else 0)
        self.collection_selector.blockSignals(False)
        self.source_collection_id = self.collection_selector.currentData()

    def source_collection_changed(self, index):
        self.source_collection_id = self.collection_selector.currentData()
        self.populate_tree()
        if self.source_collection_id:
            self.window.state['storyline_source_collection'] = self.source_collection_id
            self.window.save_timer.start(100)
    ...
    def populate_tree(self):
        ...
        self.item_map.clear()
        if not project:
            self.page_title.setText('Timeline')
            ...
            timeline.ensure_project_editing(project)
            collection_id = getattr(self, 'source_collection_id', None)
            records = release121.collection_records(self.window.state, collection_id) if collection_id else []
            self.page_title.setText('Timeline')
            has_files = bool(records)
            self.page_subtitle.setText('Stack footage, shape the cut, and send finished sequences to Delivery.')
            for record in records:
                source = str(record.get('source', ''))
                name = Path(source).name or record.get('name', 'Untitled media')
                online = Path(source).is_file()
                item = QTreeWidgetItem([name, duration_text(record.get('duration', 0)), 'Ready' if online else 'Offline'])
                item.setData(0, Qt.UserRole, record.get('id'))
                item.setToolTip(0, source)
                frame = next((frame for frame in record.get('frames', [])
                              if (Path(frame.get('path') or '') if frame.get('path') else Path(record.get('output', '')) / frame.get('file', '')).is_file()), None)
                if frame:
                    frame_path = Path(frame.get('path') or '') if frame.get('path') else Path(record.get('output', '')) / frame.get('file', '')
                    item.setIcon(0, QIcon(str(frame_path)))
                payload = {'path': source, 'name': name, 'duration': record.get('duration', 0),
                           'audio': bool(record.get('segments')), 'status': 'Ready' if online else 'Offline'}
                item.setData(0, Qt.UserRole + 1, payload)
                if not online:
                    item.setForeground(0, QColor('#777777'))
                self.tree.addTopLevelItem(item)
                self.item_map[(collection_id, source)] = item
            if not has_files:
                placeholder = QTreeWidgetItem(['This collection has no indexed media', '', ''])
                placeholder.setDisabled(True)
                self.tree.addTopLevelItem(placeholder)
        self.tree.blockSignals(False)
        if hasattr(self, 'media_grid'):
            self.media_grid.blockSignals(True)
            self.media_grid.clear()
            if project and collection_id and records:
                for record in records:
                    source = str(record.get('source', ''))
                    name = Path(source).name or record.get('name', 'Untitled media')
                    online = Path(source).is_file()
                    frame = next((frame for frame in record.get('frames', [])
                                  if (Path(frame.get('path') or '') if frame.get('path') else Path(record.get('output', '')) / frame.get('file', '')).is_file()), None)
                    frame_path = Path(frame.get('path') or '') if frame and frame.get('path') else (Path(record.get('output', '')) / frame.get('file', '') if frame else None)
                    payload = {'path': source, 'name': name, 'duration': record.get('duration', 0),
                               'audio': bool(record.get('segments')), 'status': 'Ready' if online else 'Offline'}
                    grid_item = QListWidgetItem(name)
                    grid_item.setData(Qt.UserRole, payload)
                    grid_item.setToolTip(f"{name}\n{duration_text(record.get('duration', 0))}")
                    if frame_path and frame_path.is_file():
                        grid_item.setIcon(QIcon(str(frame_path)))
                    else:
                        grid_item.setIcon(self._generic_thumbnail_icon())
                    if not online:
                        grid_item.setForeground(QColor('#777777'))
                    self.media_grid.addItem(grid_item)
            self.media_grid.blockSignals(False)
        self.update_summary()

    def _generic_thumbnail_icon(self):
        """Return a 96x56 placeholder icon for grid items without a video frame to maintain symmetry."""
        if not hasattr(self, '_cached_generic_thumb'):
            pix = QPixmap(96, 56)
            pix.fill(QColor('#161a1d'))
            p = QPainter(pix)
            p.setRenderHint(QPainter.Antialiasing)
            p.setPen(QPen(QColor('#262d33'), 1))
            p.setBrush(QColor('#1a2026'))
            p.drawRoundedRect(4, 4, 88, 48, 3, 3)
            p.setPen(QPen(QColor('#4d5963'), 1.5))
            p.setBrush(QColor('#252f38'))
            p.drawEllipse(26, 16, 8, 8)
            path = QPainterPath()
            path.moveTo(14, 42)
            path.lineTo(34, 26)
            path.lineTo(48, 38)
            ...
            p.drawPath(path)
            p.end()
            self._cached_generic_thumb = QIcon(pix)
        return self._cached_generic_thumb

    def grid_item_changed(self, current, previous=None):
        self.update_summary()
        if not current:
            return
    ...
    def set_media_symbol_mode(self, is_grid):
        if hasattr(self, 'media_stack') and hasattr(self, 'media_grid'):
            self.media_stack.setCurrentWidget(self.media_grid if is_grid else self.tree)
        if hasattr(self, 'grid_mode_btn') and hasattr(self, 'list_mode_btn'):
            (self.grid_mode_btn if is_grid else self.list_mode_btn).setChecked(True)
        if hasattr(self, 'window') and hasattr(self.window, 'state'):
            self.window.state.setdefault('settings', {})['sequence_media_grid'] = bool(is_grid)
            self.window.save_timer.start(250)

    def set_media_view_mode(self, checked):
        self.set_media_symbol_mode(checked)

        if hasattr(self, 'media_stack') and hasattr(self, 'media_grid'):
            self.media_stack.setCurrentWidget(self.media_grid if checked else self.tree)
        if hasattr(self, 'window') and hasattr(self.window, 'state'):
            self.window.state.setdefault('settings', {})['sequence_media_grid'] = bool(checked)
            self.window.save_timer.start(250)
    ...
    def tree_item_changed(self, item, previous=None):
        ...
        if not item:
            return
        video = item.data(0, Qt.UserRole + 1)
        if video:
            self.inspect_source(video)
    ...
    def update_summary(self):
        project = self.current()
        collection_id = getattr(self, 'source_collection_id', None)
        files = release121.collection_records(self.window.state, collection_id) if collection_id else []
        if project:
            sequence = self.active_sequence()
        ...
        self.tree.setVisible(bool(project and collection_id))
        ...
        self.empty_action_button.setText('Select Collection')
    ...
    def set_busy(self, busy):
        for widget in (self.projects, self.tree, self.add_media_button):
            widget.setEnabled(not busy)
        if not busy:
            self.update_summary()
...
class Window(QMainWindow):
    def __init__(self):
        ...
        brand_name.installEventFilter(self)
        brand_name.setCursor(Qt.PointingHandCursor)
        ...
        new_coll_btn.setStyleSheet(
            ...
            'color: #888888; font-size: 16px; font-weight: bold; padding: 0; } '
            'QPushButton:hover { background: #222222; border: 1px solid #444444; border-radius: 4px; color: #ffffff; } '
            'QPushButton:pressed { background: #2a2a2a; border: 1px solid #555555; border-radius: 4px; color: #ffffff; }'
        )
        new_coll_btn.setToolTip('Create a new collection')
        self.new_coll_btn = new_coll_btn
        ...
        beta_row = QWidget()
        beta_row.setObjectName('transparentRow')
        beta_layout = QHBoxLayout(beta_row)
        beta_layout.setContentsMargins(6, 3, 6, 5)
        beta_layout.setSpacing(7)
        beta_icon = QLabel()
        beta_icon.setPixmap(tile_icon('flask', '#7f969b').pixmap(16, 16))
        beta_icon.setToolTip('Beta')
        beta_layout.addWidget(beta_icon)
        beta_layout.addSpacing(4)
        self.beta_toggle = ToggleSwitch()
        self.beta_toggle.setChecked(bool(self.state['settings'].get('beta_features', False)))
        self.beta_toggle.toggled.connect(self.set_beta_features)
        beta_layout.addWidget(self.beta_toggle)
        beta_layout.addStretch()
        beta_row.setToolTip('Beta')
        side.addWidget(beta_row)
        ...
        self.sidebar = sidebar
        self.cursor_aura = CursorAura(sidebar, self)
        self.apply_beta_visibility()
        self.navigate(0)
    ...
    def navigate(self, index):
        if index == 2 and not self.state['settings'].get('beta_features', False):
            index = 1
    ...
    def eventFilter(self, watched, event):
        if watched in (getattr(self, 'brand_mark', None), getattr(self, 'brand_name', None)) and event.type() == QEvent.MouseButtonDblClick:
            self.open_transcape()
            return True
    ...
    def set_beta_features(self, enabled):
        self.state['settings']['beta_features'] = bool(enabled)
        self.apply_beta_visibility()
        self.save_timer.start(100)

    def apply_beta_visibility(self):
        enabled = bool(self.state['settings'].get('beta_features', False))
        if hasattr(self, 'beta_toggle') and self.beta_toggle.isChecked() != enabled:
            self.beta_toggle.blockSignals(True)
            self.beta_toggle.setChecked(enabled)
            self.beta_toggle.blockSignals(False)
        if hasattr(self, 'paper_nav_button'):
            self.paper_nav_button.setVisible(enabled)
        for page_name in ('results', 'collection_page'):
            page = getattr(self, page_name, None)
            if page and hasattr(page, 'set_beta_enabled'):
                page.set_beta_enabled(enabled)
        if hasattr(self, 'projects_page') and hasattr(self.projects_page, 'set_beta_enabled'):
            self.projects_page.set_beta_enabled(enabled)
        if hasattr(self, 'visual_settings_card'):
            self.visual_settings_card.setVisible(enabled)
        if hasattr(self, 'visual_cache_button'):
            self.visual_cache_button.setVisible(enabled)
        if not enabled and hasattr(self, 'pages') and self.pages.currentIndex() == 2:
            self.navigate(1)
    ...
    def show_collection_context_menu(self, pos, collection_id):
        ...
        if a_rename and action == a_rename:
            ...
            records = release121.collection_records(self.state, collection_id)
            self.results.export_files(None, records, title=f"Export '{coll.get('name', 'Collection')}'")
    ...
    def make_queue(self):
        ...
        self.add_button = QueueAddButton('+ Add media', self.add_media)
    ...
    def make_settings(self):
        ...
        self.visual_settings_card = visual_card
        ...
        self.visual_cache_button = button('Clear AI preview cache', self.clear_visual_detail_cache)
        storage.addWidget(self.visual_cache_button)
        storage.addStretch()
    ...
    def first_setup(self):
        if not core.model_ready('tiny'):
            self.download('tiny')
        elif self.state['settings'].get('beta_features', False) and not core.visual_model_ready():
    ...
    def add_media(self):
        ...
        picker = MediaPickerDialog(self)
        if picker.exec() == QDialog.Accepted:
            self.scan_media_paths(picker.media_paths())
    ...
    def selection_changed(self, item, column):
        if column != 0:
            return
        self._last_queue_toggled_item = item
        self._last_queue_toggled_time = time.time()
        # Qt propagates folder tri-state checkboxes to children.
        for folder in self.state['folders']:
            for v in folder['files']:
                child = self.item_map.get((folder['id'], v['path']))
                if child:
    ...
    def on_queue_item_clicked(self, item, column):
        if column == 0 and item:
            last_item = getattr(self, '_last_queue_toggled_item', None)
            last_time = getattr(self, '_last_queue_toggled_time', 0)
            if last_item is item and (time.time() - last_time) < 0.25:
                return  # Handled directly by Qt indicator
            # Generous click capture area around the checkbox
            pos = self.tree.viewport().mapFromGlobal(QCursor.pos())
            vrect = self.tree.visualItemRect(item)
            # Use depth-aware click area to capture indented checkboxes perfectly
            depth = 1 if item.parent() else 0
            indent = self.tree.indentation()
            checkbox_x = vrect.x() + (depth * indent)
            if vrect.isValid() and checkbox_x - 10 <= pos.x() <= checkbox_x + 65:
    ...
    def update_stats(self):
        ...
        if hasattr(self.add_button, 'setAttention'):
            self.add_button.setAttention(not self.state['folders'] and not self.batch and not self.scanning)
        if not self.batch:
    ...
    def set_busy(self, busy):
        for widget in (self.add_button, self.recursive, self.remove, self.rescan, self.up, self.down):
            widget.setEnabled(not busy)
        if not busy:
            self.update_queue_controls()
        self.process.setEnabled(not busy and any(v['selected'] for f in self.state['folders'] for v in f['files']))
        if hasattr(self.add_button, 'setAttention'):
            self.add_button.setAttention(not busy and not self.state['folders'])
    ...
    def start_batch(self, folders, options, context):
        ...
        self.progress.setRange(0, 100)
        self.progress.setValue(0)
        self.progress.setFormat('0% of total batch    ETA Calculating...')
        self.progress_detail.setText('0 of 0 videos')
        self.projects_page.progress.setRange(0, 100)
        self.projects_page.progress.setValue(0)
        self.projects_page.progress.setFormat('0% of project batch    ETA Calculating...')
    ...
    def batch_event(self, folder_id, path, kind, value):
        ...
        if kind in ('status', 'error'):
            ...
            eta_str = value.get('eta', 'Calculating...')
            progress.setFormat(f"{value['percent']}% of {'project' if project else 'total'} batch    ETA {eta_str}")
