...
## 1.23 current journey

Beta features (Paper Edit and Visual Indexing) are hidden until enabled via the toggle, which defaults to off. A subtle cursor aura follows the pointer, becoming stronger over the sidebar. Transcape now uses force-directed physics with cluster seeding and direct node manipulation. The processing overlay can be dismissed by clicking outside, and the library preview uses a resizable splitter.

## 1.22 current journey

Library searches globally; Collections own a separate footage workspace.
Old projects are presented as Collections. Collection actions open Paper Edit
or Storyline. Library selection assigns footage using its bottom destination
picker; collection pages omit this assignment action. Both share preview tools.

Paper Edit opens Search beside script with a continuous manuscript, keeps
source timing behind text, and updates one linked Storyline sequence. Passage
Order controls inclusion, restoration, and speech order while ordinary wording
changes remain separate from media timing. Secondary preview panels collapse at
the narrow supported width so the manuscript stays usable.

Storyline owns the visual assembly. Its symbol-led Selection, Track Select,
Ripple, Razor, Hand, and Zoom tools retain familiar shortcuts with a text-focus
guard. Manual Storyline layers survive Paper synchronization. Export offers
Premiere-compatible XML or a collected project folder. Double-clicking the logo
opens Transcape; Escape returns. See RELEASE-1.22.md for verified boundaries.
