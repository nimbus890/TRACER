...
## 1.23 current direction

The Beta toggle is placed above the GPU meters and its state persists. The library features a resizable preview splitter for better layout control. Storyline uses a collection dropdown with thumbnails for media selection. A cursor aura layer enhances interaction feedback. The processing overlay dimensions are refined for better balance.

## 1.22 current direction

Calm near-black editing workspace; footage, manuscript and timeline dominate.
Retain the approved Tracer mark and Midday-derived neutral hierarchy. Larger
14px primary navigation labels and 20px symbols improve scanning. Collection
assignment belongs beneath the Library list; Open Results and Show in Explorer
are prominent preview actions. Collection pages retain the preview structure.

Manuscript uses one 18px continuous text surface with generous line spacing;
timing details are contextual. At narrow widths the manuscript and search remain
while secondary previews collapse. Storyline media uses a searchable, flattened
single-column bin and compact Add/Insert actions. Timeline tools use one native
monochrome symbol language; shortcut letters live in tooltips rather than on the
buttons themselves.
Transcape uses black space, thin connections and restrained data-category colors.
Desktop 1440×900 and narrow 1100×720 renders were inspected; evidence and limits
are recorded in RELEASE-1.22.md and VALIDATION.md.

## 1.18 interaction refinement

Keep a calm graphite editing desk, with the user's footage and timeline dominant and navigation chrome compact. Borrow Premiere's preview/timeline hierarchy, Penpot's contextual toolbar grouping, and Fluent's familiar direct manipulation. Landscape places the wide preview above the timeline; Portrait gives the preview a narrow tall column on the left. Orientation symbols sit beside Storyline. Layout remains available and Export Queue opens as a top-right popup. The preview can shrink or collapse, and empty sequences expose controls immediately. Search-origin glyphs are 22 px, unboxed, with cyan transcript, green visual, and yellow filename colors beside result counts. Transcript direct/indirect counts only appear for transcript hits. Desktop and 1100 px layouts, preview collapse/recovery, and native Windows result seeking are verified for this release.
