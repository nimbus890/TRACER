...
class AppInteractionTests(unittest.TestCase):
    ...
    def test_add_media_uses_media_picker_dialog(self):
        class Target:
            scanning = False
            batch = None
        ...
        mock_picker.exec.return_value = app.QDialog.Accepted
        mock_picker.media_paths.return_value = ['one.mp4', 'two.mov']
        with patch('app.MediaPickerDialog', return_value=mock_picker) as picker:
