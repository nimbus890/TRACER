...
class LibraryPickerDialog(QDialog):
    ...
    def refresh(self):
        ...
        for record in self.records:
            haystack = ' '.join([
                Path(record.get('source', '')).name,
                ' '.join(segment.get('text', '') for segment in record.get('segments', [])),
...
class PaperSegmentRow(QFrame):
    ...
    def __init__(self, segment, parent=None):
        ...
        self.speaker.setMaximumWidth(115)
        ...
        self.highlight.setIconSize(QSize(24, 24))
    ...
    def fit_text(self):
        ...
        self.text.setFixedHeight(max(62, min(300, height)))
...
class PaperDocument(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.layout_box.setContentsMargins(24, 18, 24, 36)
    ...
    def set_document(self, document):
        ...
        self.layout_box.addStretch()
        self.setMinimumHeight(max(620, len(self.rows) * 112 + 70))
    ...
    def apply_search(self, query):
        ...
        for row in self.rows:
            ...
            if match:
                row.setStyleSheet('QFrame#paperSegment { background: #3a2a24; }')
            else:
                row.setStyleSheet('')
...
class PaperEditPage(QWidget):
    def __init__(self, window):
        ...
        top = QHBoxLayout()
        heading = QVBoxLayout()
        eyebrow = label('WRITER WORKSPACE', 'paperEyebrow')
        heading.addWidget(eyebrow)
        title = label('Paper Edit', 'paperTitle')
        heading.addWidget(title)
        top.addLayout(heading)
        top.addSpacing(24)
        self.search = QLineEdit()
        self.search.setPlaceholderText('Find a line in this transcript…')
        self.search.setMaximumWidth(360)
        self.search.setMinimumWidth(220)
        self.search.textChanged.connect(lambda value: self.paper.apply_search(value))
        top.addWidget(self.search)
        top.addStretch()
        self.analysis = label('Choose footage from the Library', 'paperSubtle')
        top.addWidget(self.analysis)
        library_button = button('Library', self.add_from_library)
        library_button.setIcon(paper_icon('library'))
        top.addWidget(library_button)
        self.export_button = button('Export script', self.export_script)
        self.export_button.setIcon(paper_icon('export', '#ffffff'))
        self.export_button.setObjectName('paperPrimary')
        top.addWidget(self.export_button)
        layout.addLayout(top)
        workspace = QSplitter(Qt.Horizontal)
        rail = QFrame()
        rail.setObjectName('paperRail')
        rl = QVBoxLayout(rail)
        rl.setContentsMargins(10, 10, 10, 10)
        rail_label = label('TRANSCRIPTS', 'paperEyebrow')
        rl.addWidget(rail_label)
        self.documents = QListWidget()
        self.documents.currentItemChanged.connect(self.select_document)
        rl.addWidget(self.documents, 1)
        add = button('+ Add from Library', self.add_from_library)
        rl.addWidget(add)
        ...
        tools = QHBoxLayout()
        tools.addWidget(label('SCRIPT', 'paperEyebrow'))
        tools.addStretch()
        self.pen = QToolButton()
        self.pen.setIcon(paper_icon('pen', '#c64b35'))
        ...
        self.marker.setIcon(paper_icon('marker', '#a36b00'))
        self.marker.setIconSize(QSize(24, 24))
        ...
        sequence_head = QHBoxLayout()
        sequence_head.addWidget(label('PAPER SEQUENCE · drag passages to reorder', 'paperEyebrow'))
        sequence_head.addStretch()
        cl.addLayout(sequence_head)
        self.sequence = QListWidget()
        self.sequence.setFlow(QListWidget.LeftToRight)
        self.sequence.setWrapping(False)
        self.sequence.setDragDropMode(QAbstractItemView.InternalMove)
        self.sequence.setMaximumHeight(92)
        self.sequence.model().rowsMoved.connect(self.sequence_reordered)
        cl.addWidget(self.sequence)
        workspace.addWidget(centre)
        side = QFrame()
        side.setObjectName('paperCard')
        sl = QVBoxLayout(side)
        sl.setContentsMargins(10, 10, 10, 10)
        sl.addWidget(label('SOURCE PREVIEW', 'paperEyebrow'))
        ...
        controls = QHBoxLayout()
        play = QToolButton()
        play.setIcon(tile_icon('play', '#e8e3da'))
        play.clicked.connect(self.play_source)
        controls.addWidget(play)
        self.clock = label('00:00:00', 'paperSubtle')
        controls.addWidget(self.clock)
        controls.addStretch()
        sl.addLayout(controls)
        sl.addWidget(label('VISUAL REFERENCES', 'paperEyebrow'))
        self.frames = QListWidget()
        self.frames.setViewMode(QListWidget.IconMode)
        self.frames.setIconSize(QSize(150, 84))
        self.frames.setResizeMode(QListWidget.Adjust)
        self.frames.setSpacing(6)
        sl.addWidget(self.frames, 1)
        workspace.addWidget(side)
        workspace.setSizes([205, 720, 285])
        layout.addWidget(workspace, 1)
        self.player.positionChanged.connect(lambda value: self.clock.setText(duration_text(value / 1000)))
        self.refresh()
    ...
    def select_document(self, item, previous=None):
        ...
        self.player.stop()
        if self.document and Path(self.document.get('source', '')).is_file():
    ...
    def populate_frames(self):
        ...
        for frame in self.record.get('frames', [])[:30]:
            ...
            if path and Path(path).is_file():
                item = QListWidgetItem(QIcon(path), duration_text(frame.get('time', 0)))
                self.frames.addItem(item)
...
class ProjectsPage(LegacyProjectsPage):
    ...
    def __init__(self, window):
        ...
        heading = QHBoxLayout()
        words = QVBoxLayout()
        words.addWidget(label('STORY SEQUENCE', 'eyebrow'))
    ...
    def build_media_panel(self):
        ...
        self.add_media_button = button('+ Open Video Library', self.add_library_media, True)
        ...
        self.insert_media_button = button('Insert selected into sequence', self.insert_selected_media)
    ...
    def sequence_tick(self):
        ...
        if position >= timeline.sequence_duration(sequence):
            self.timeline_canvas.playhead = timeline.sequence_duration(sequence)
            self.timeline_canvas.update()
            self.stop_sequence_playback()
            return
        self.timeline_canvas.playhead = position
        active_clip = next((clip for track in sequence.get('tracks', []) if track.get('type') == 'video' and track.get('visible', True)
                            for clip in track.get('clips', []) if float(clip.get('start', 0)) <= position < float(clip.get('end', 0))), None)
        if active_clip and active_clip.get('id') != self.playing_clip_id:
            ...
            self.player.setPosition(round((float(active_clip.get('source_in', 0)) + position - float(active_clip['start'])) * 1000))
            self.player.play()
    ...
    def update_summary(self):
        ...
        for widget in (self.progress, self.progress_detail, self.processing_box):
            widget.hide()
        self.editor_stack.setCurrentIndex(1 if has_media else 0)
...
class Window(QMainWindow):
    def __init__(self):
        ...
        self.nav = []
        navigation = (
            ('Paper Edit', 'paper'), ('Storyline', 'storyline'), ('Library', 'library'),
        ...
        for i, (name, icon_name) in enumerate(navigation):
            ...
            b.setCheckable(True)
            b.setToolTip((
                'Write and shape the story from transcripts', 'Simple stackable sequence timeline',
                'Searchable video library', 'Folder processing queue', 'Settings')[i])
            self.nav.append(b)
            side.addWidget(b)
        side.addStretch()
        self.usage_labels = {}
        ...
        side.addSpacing(10)
        version = label('Tracer · 1.15', 'subtle')
    ...
    def navigate(self, index):
        ...
        if index != 0 and hasattr(self, 'paper_edit_page'):
            self.paper_edit_page.player.pause()
        if index != 1 and hasattr(self, 'projects_page'):
    ...
    def closeEvent(self, event):
        ...
        self.paper_edit_page.player.stop()
