...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
    ...
    QSizePolicy, QTabWidget, QGridLayout, QAbstractButton, QSizeGrip)
...
QTreeWidget::indicator { width: 17px; height: 17px; border-radius: 4px; border: 1.5px solid #50555a; background: #14171a; }
QTreeWidget::indicator:hover { border-color: #8a9096; background: #202428; }
QTreeWidget::indicator:checked { border: 1.5px solid #ff7a1a; background: #ff7a1a; }
...
class MediaTableDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        if index.column() == 0:
            ...
            if checked:
                painter.setPen(QPen(QColor('#ff7a1a'), 1.5))
                painter.setBrush(QBrush(QColor('#ff7a1a')))
                ...
                painter.setPen(QPen(QColor('#555a62'), 1.5))
                painter.setBrush(QBrush(QColor('#181b1f')))
                painter.drawRoundedRect(box_rect, 4, 4)
            painter.restore()
            return
        elif index.column() != 1:
...
class CardResizeGrip(QSizeGrip):
    """Custom size grip that resizes a centered modal card inside an overlay."""
    ...
    def mouseMoveEvent(self, event):
        if self._dragging:
            ...
            max_w = (overlay.width() - 40) if overlay else 1800
            max_h = (overlay.height() - 40) if overlay else 1200
            new_w = max(680, min(max_w, self._start_size.width() + delta.x() * 2))
            new_h = max(440, min(max_h, self._start_size.height() + delta.y() * 2))
            self.target_card.setFixedSize(new_w, new_h)
            event.accept()
...
class LibraryPickerDialog(QDialog):
    ...
    def __init__(self, records, parent=None):
        ...
        self.card = QFrame()
        self.card.setObjectName('libraryCard')
        self.card.setStyleSheet('QFrame#libraryCard { background: #141414; border: 1px solid #2a2a2a; border-radius: 9px; }')
    ...
    def showEvent(self, event):
        ...
        if parent_win and parent_win != self:
            ...
            card_w = max(680, min(1400, int(parent_win.width() * 0.84)))
            card_h = max(440, min(960, int(parent_win.height() * 0.82)))
            self.card.setFixedSize(card_w, card_h)
        else:
            self.setGeometry(100, 100, 1060, 720)
            self.card.setFixedSize(860, 580)
    ...
    def paintEvent(self, event):
        ...
        if hasattr(self, '_cursor_point') and self._cursor_point is not None:
            if hasattr(self, 'card') and not self.card.geometry().contains(self._cursor_point):
                gradient = QRadialGradient(self._cursor_point, 220)
                gradient.setColorAt(0, QColor(95, 220, 229, 32))
                gradient.setColorAt(.35, QColor(95, 220, 229, 14))
                gradient.setColorAt(1, QColor(0, 0, 0, 0))
                painter.setPen(Qt.NoPen)
                painter.setBrush(QBrush(gradient))
                painter.drawEllipse(self._cursor_point, 220, 220)
...
class LegacyProjectsPage(QWidget):
    ...
    def insert_selected_media(self):
        if hasattr(self, 'media_stack') and self.media_stack.currentWidget() == getattr(self, 'media_grid', None):
            item = self.media_grid.currentItem()
        ...
        if payload:
            ...
            self.status.setText('Select a video in Collection media first.')
...
class ProjectsPage(LegacyProjectsPage):
    ...
    def build_empty_editor(self):
        ...
        self.paper_import_button.setToolTip('Go back to Paper Edit, select passages, then click → Storyline to send them here')
    ...
    def build_media_panel(self):
        ...
        self.insert_media_button = button('Insert', self.insert_selected_media)
        self.insert_media_button.setToolTip('Insert selected media into timeline')
        self.insert_media_button.setFixedWidth(54)
