...
class ProjectsPage(LegacyProjectsPage):
    ...
    def populate_tree(self):
        ...
        if not project:
            ...
            self.page_subtitle.setText('Stack footage, shape the cut, and send finished sequences to Delivery.')
            for folder in project.get('folders', []):
                videos = folder.get('files', [])
                if len(videos) == 1 and str(folder.get('path', '')).casefold() == str(videos[0].get('path', '')).casefold():
                    video = videos[0]
                    item = QTreeWidgetItem([video['name'], duration_text(video.get('duration', 0)), video.get('status', 'Ready')])
                    item.setData(0, Qt.UserRole, (folder['id'], video['path']))
                    item.setToolTip(0, video['path'])
                    item.setToolTip(2, video.get('error', ''))
                    self.tree.addTopLevelItem(item)
                    if video.get('status') == 'Unreadable':
                    ...
                    self.item_map[(folder['id'], video['path'])] = item
                    continue
                parent = QTreeWidgetItem([folder['name'], '', ''])
                parent.setData(0, Qt.UserRole, (folder['id'], None))
                parent.setToolTip(0, folder['path'])
