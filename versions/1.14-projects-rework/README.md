...
Double-click **Launch Tracer 1.14 Projects Rework.vbs** in this folder to open this preserved source snapshot. A future export build will be created as **dist/Tracer/Tracer.exe**.
...
The original editable 1.14 source is preserved under **versions/1.14**, its post-milestone Upgrade Pass under **versions/1.14-upgrade-pass**, and the subsequent Projects workspace rework under **versions/1.14-projects-rework**. Earlier snapshots remain under **versions**. The 1.14 installer is intentionally deferred until an exportable build is requested; completed earlier TransPro installers remain in **release**.
...
Projects is now an editing room rather than a second Queue. The selected project and one unified, draggable media tree sit on the left; source playback and processed transcript inspection sit above the active sequence; and the multitrack story timeline fills the main workspace. Empty projects replace the disabled editor with one clear Add media action. Delivery and its export queue open as an on-demand drawer instead of permanently taking editing space.

Each project can create, rename, duplicate, reorder, and close sequence tabs. One Add media picker accepts folders, individual videos, or both. Drag a video directly from Project media onto stacked video or audio tracks, double-click it, or use Insert selected. Video with audio creates linked picture and sound clips. Clips can be moved between compatible tracks, split, cut, copied, pasted, duplicated, deleted, undone, and redone. The timeline supports playhead markers, zoom, fit, extra tracks, and track visibility, mute, solo, and lock states. Sequence settings store frame rate and frame size.

The 1.14 Upgrade Pass keeps linked picture and sound in sync while dragging, makes locked tracks consistently protect linked edits, adds discoverable editing shortcuts, and keeps the toolbars readable on narrower desktop windows. Empty timelines cannot be queued or exported, collected-media reports call out offline sources, and Premiere XML writes replace their destination atomically so an interrupted export cannot masquerade as a complete file.
