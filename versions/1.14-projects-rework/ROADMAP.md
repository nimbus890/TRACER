...
## Patterns to carry over from Unpolished

The Unpolished specification lives in the ChatGPT project named `idk 1`. Tracer should borrow its strongest representation ideas while remaining an editor-focused product:

- Separate **Observation**, **Interpretation**, **Creative Principle**, and **Editor Intent**. For example: “two people at opposite frame edges” is evidence; “emotional separation” is an interpretation; “use spatial distance to express conflict” is a transferable principle; “keep this only as a reaction cutaway” is editor intent.
- Introduce editable **Visual Traits** attached to a shot or time range. A user can keep only the lighting, composition, camera movement, palette, atmosphere, performance, or production-design quality they care about.
- Let every Story Editor item declare its intended contribution: dialogue, reaction, B-roll, transition, atmosphere, evidence, visual metaphor, or reference only.
- Clearly distinguish machine-inferred metadata from editor-authored choices. Editor intent overrides AI interpretation, and important traits can be corrected or locked.
- Preserve provenance and confidence: every suggestion should lead back to the exact source video and time range that supports it.
- Use selected traits when proposing visual alternatives. “Same emotional effect through a different image” is more useful than generic visual similarity.
- Allow a story draft to develop a small visual thesis and avoid-rules, but do not turn Tracer into Unpolished's broader visual-learning product.

The first non-AI Story Editor can already support manual traits and intent. The beta can propose observations, principles, relationships, and alternatives later without changing the underlying project structure.
