...
class ProjectsPage(LegacyProjectsPage):
    ...
    def __init__(self, window):
        ...
        self.page_title = label('Projects', 'title')
        words.addWidget(self.page_title)
        self.page_subtitle = label('Choose a project or create one to begin.', 'subtle')
    ...
    def build_empty_editor(self):
        ...
        layout.addStretch()
        self.empty_title = label('Add media to start editing', 'title')
        self.empty_title.setAlignment(Qt.AlignCenter)
        layout.addWidget(self.empty_title)
        self.empty_copy = label(
            'Choose folders, individual videos, or both. Tracer keeps every source linked without moving it.',
            'subtle')
        ...
        self.empty_action_button = button('+ Add media', self.empty_project_action, True)
    ...
    def build_media_panel(self):
        ...
        ll.addWidget(label('SAVED PROJECTS', 'eyebrow'))
        self.projects = QListWidget()
        self.projects.setMaximumHeight(96)
        self.projects.currentItemChanged.connect(self.project_changed)
        ...
        self.add_media_button = button('+ Add media', self.window.add_project_media, True)
        ...
        self.media_hint = label('Check media to process · drag or double-click a video to edit', 'subtle')
    ...
    def populate_tree(self):
        ...
        self.item_map.clear()
        if not project:
            self.page_title.setText('Projects')
            self.page_subtitle.setText('Choose a project or create one to begin.')
            self.summary.setText('No project selected.')
        else:
            timeline.ensure_project_editing(project)
            self.page_title.setText(project['name'])
            has_files = any(folder.get('files') for folder in project.get('folders', []))
            self.page_subtitle.setText(
                'Find the source, shape the sequence, then open Delivery when it is ready.'
                if has_files else 'This project is ready for its first source.')
            for folder in project.get('folders', []):
                parent = QTreeWidgetItem([folder['name'], '', ''])
                parent.setData(0, Qt.UserRole, (folder['id'], None))
                parent.setToolTip(0, folder['path'])
    ...
    def update_summary(self):
        ...
        if project:
            ...
            sequence_text = (f"\n{sequence['name']} · {duration_text(timeline.sequence_duration(sequence))}"
                             if sequence else '')
            self.summary.setText(f"{len(files)} media · {len(selected)} selected{sequence_text}")
            if not self.window.batch:
                self.progress_detail.setText(f"{len(selected)} {'video' if len(selected) == 1 else 'videos'} ready")
        ready = bool(project and selected and not self.window.batch and not self.window.scanning)
        self.process.setEnabled(ready)
        ...
        show_media_workflow = has_media or self.window.scanning or bool(self.window.batch)
        self.tree.setVisible(bool(project))
        for widget in (self.media_hint, self.insert_media_button, self.status,
                       self.progress, self.progress_detail, self.processing_box):
            widget.setVisible(show_media_workflow)
        ...
        if project:
            self.empty_title.setText('Add media to start editing')
            self.empty_copy.setText('Choose folders, individual videos, or both. Tracer keeps every source linked without moving it.')
            self.empty_action_button.setText('+ Add media')
        else:
            self.empty_title.setText('Create your first project')
            self.empty_copy.setText('Projects keep media, sequences, transcripts, and delivery history together.')
            self.empty_action_button.setText('+ New project')
    ...
    def set_busy(self, busy):
        for widget in (self.projects, self.tree, self.add_media_button, self.remove_source):
            widget.setEnabled(not busy)
        if busy:
            for widget in (self.status, self.progress, self.progress_detail, self.processing_box):
                widget.show()
        if not busy:
            self.update_summary()
