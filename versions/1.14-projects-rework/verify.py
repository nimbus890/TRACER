...
def ui():
    ...
    picker = app.MediaPickerDialog(window)
    assert picker.fileMode() == app.QFileDialog.ExistingFiles
    assert picker.testOption(app.QFileDialog.DontUseNativeDialog)
    assert picker.testOption(app.QFileDialog.ReadOnly)
    picker.close()
    ...
    options.scene_mode.setChecked(False)
    QTest.qWait(220)
    application.processEvents()
    ...
    video_clip = project_sequence['tracks'][1]['clips'][0]
    project_sequence['tracks'][2]['locked'] = True
    window.projects_page.timeline_canvas.selected_id = video_clip['id']
    window.projects_page.sequence_command('delete')
    assert video_clip in project_sequence['tracks'][1]['clips'], 'A locked linked track must protect the edit'
    assert 'Unlock' in window.projects_page.status.text()
    project_sequence['tracks'][2]['locked'] = False
    window.projects_page.status.setText('Ready. Drag media into a sequence or process unprepared sources.')
    ...
    application.processEvents()
    assert window.projects_page.editor_stack.currentIndex() == 0
    assert not window.projects_page.delivery_toggle.isEnabled()
    assert window.projects_page.empty_action_button.text() == '+ Add media'
    window.grab().save(str(core.ROOT / 'verification' / 'projects-empty.png'))
    window.projects_page.refresh('ui-project')
    window.navigate(2)
    assert len(window.nav) == 4 and all(button.text() != 'Footage search' for button in window.nav)
    ...
    if window.results.list.count():
        ...
        open_width = window.results.media_split.width()
