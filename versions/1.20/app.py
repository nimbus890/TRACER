...
class ResultsPage(QWidget):
    ...
    def set_active_collection(self, collection_id):
        release120.migrate(self.window.state)
    ...
    def on_add_media(self):
        if self.active_collection_id != 'all':
            ...
            if picker.exec() == QDialog.Accepted:
                release120.attach(self.window.state, self.active_collection_id, picker.selected_records())
    ...
    def on_export_project(self):
        ...
        if dest:
            self.export_files(dest, release120.collection_records(self.window.state, self.active_collection_id))
        return
    ...
    def on_add_to_collection(self):
        ...
        if target and target != '__new_project__':
            release120.attach(self.window.state, target, [r for r in self.records if r.get('id') in self.checked_ids])
            self.window.refresh_collections_sidebar()
            self.window.save_timer.start(100)
    ...
    def open_collection_editor(self, kind):
        records = release120.collection_records(self.window.state, self.active_collection_id)
        if self.checked_ids:
        ...
        release120.attach(self.window.state, collection['id'], records)
...
class CollectionPickerDialog(QDialog):
    ...
    def apply_add(self):
        if self.selected_collection_id:
            release120.migrate(self.state)
            release120.attach(self.state, self.selected_collection_id,
...
class PaperEditPage(QWidget):
    ...
    def open_storyline(self):
        if self.document:
            collection, sequence = release120.sync_paper(self.window.state, self.document)
            self.window.projects_page.refresh(collection['id'])
    ...
    def document_changed(self):
        ...
        self.update_sequence()
        if self.document:
            release120.sync_paper(self.window.state, self.document)
    ...
    def export_script(self):
        ...
        if chosen == premiere:
            ...
            if target:
                ...
                task = self.window.run_task(lambda notify: release120.export_paper_project(snapshot, target), self.export_finished)
...
class ProjectsPage(LegacyProjectsPage):
    ...
    def select_timeline_tool(self, name):
        ...
        self.timeline_canvas.setCursor(Qt.CrossCursor if name == 'razor' else Qt.ArrowCursor)
        for key, control in self.tool_buttons.items():
            control.setChecked(key == name)
        self.status.setText('Click a clip to cut. Drag the ruler to position the playhead.' if name == 'razor' else 'Drag clips to move them. Drag the ruler to scrub.')
    ...
    def build_editing_panel(self):
        ...
        self.edit_buttons = {}
        self.tool_buttons = {}
        for text, name in (('Select', 'select'), ('Cut', 'razor')):
...
class Window(QMainWindow):
    def __init__(self):
        ...
        if self.state.get('schema_version', 0) < 120 and (core.DATA / 'library.json').exists():
            backup = core.DATA / 'library-before-1.20.json'
    ...
    def show_collection_context_menu(self, pos, collection_id):
        ...
        if a_rename and action == a_rename:
            ...
            if dest:
                self.results.export_files(dest, release120.collection_records(self.state, collection_id))
