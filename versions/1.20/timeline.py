...
class TimelineCanvas(QWidget):
    ...
    def mousePressEvent(self, event):
        ...
        if clip and event.button() == Qt.LeftButton and self.tool == 'razor':
            ...
            self.set_playhead((event.position().x() - TRACK_HEADER) / self.pixels_per_second, notify=True)
        ...
        if clip:
            ...
            if event.position().x() < TRACK_HEADER and event.position().y() >= RULER_HEIGHT:
                ...
                self.set_playhead((event.position().x() - TRACK_HEADER) / self.pixels_per_second,
