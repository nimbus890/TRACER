...
QDialog#processDialog { background: rgba(23, 23, 22, 247); border: 1px solid #4c4842; border-radius: 8px; }
...
class ChoiceTile(QToolButton):
    ...
    def _apply_style(self):
        ...
        palettes = {
            'audio': ((216, 195, 235), (88, 58, 105)),
            'frame': ((185, 220, 235), (35, 83, 103)),
            'ai': ((192, 226, 202), (36, 89, 56)),
        }
...
class ResultsPage(QWidget):
    def __init__(self, parent=None):
        ...
        self.audio_scope = ChoiceTile('audio', 'Audio', compact=True)
        self.frame_scope = ChoiceTile('frame', 'Frames', compact=True)
        self.visual_scope = ChoiceTile('ai', 'Visual', compact=True)
        for control in (self.audio_scope, self.frame_scope, self.visual_scope):
            control.setFixedSize(104, 36)
        ...
        self.frame_scope.setToolTip('Search video names and show videos that contain captured frames.')
    ...
    def populate(self):
        ...
        for record in reversed(self.records):
            searchable = []
            if self.frame_scope.isChecked():
                searchable.append(record['source'] + ' ' + record.get('project_name', ''))
    ...
    def open_search(self):
        if self.search_panel.isVisible() and self.search_target == 310 and self.search_panel.maximumWidth() > 0:
            return
        self.search_animation.stop()
...
class Window(QMainWindow):
    def __init__(self):
        ...
        side.addSpacing(15)
        side.addWidget(label('TransPro · 1.8', 'subtle'))
    ...
    def refresh_models(self):
        ...
        if hasattr(self, 'visual_model_state'):
            ...
            self.visual_model_state.setText(('Installed · ' if ready else 'Included with 1.8 · approximately ') + size_text(size))
