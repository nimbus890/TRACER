...
The editable 1.12 source is preserved under **versions/1.12**. Its installer is intentionally deferred until an exportable build is requested; completed earlier TransPro installers remain in **release**.

## Workflow

1. Open **Add media** and choose either **Folder** or **Individual videos**. Both choices use the native Windows Explorer picker. The folder picker adds one folder at a time; reopen it to add more. The video picker supports selecting multiple files together.
2. Newly added sources expand automatically. Check or uncheck individual videos, or the entire folder.
3. Use the arrow buttons to change the folder order. **Rescan** discovers new files. The trash icon beside Add media removes the selected video or folder from the queue without deleting anything from disk.
...
## Unified offline search in 1.12
