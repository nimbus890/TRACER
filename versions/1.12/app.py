...
QPushButton#queueDelete { background: transparent; border: 0; padding: 0; }
QPushButton#queueDelete:hover { background: #32191a; border: 0; }
...
def tile_icon(kind, colour_override=None):
    ...
    if kind == 'audio':
        ...
        painter.setPen(QPen(QColor('#f40009'), 2.3, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
...
class Window(QMainWindow):
    def __init__(self):
        ...
        side.addSpacing(15)
        side.addWidget(label('Tracer · 1.12', 'subtle'))
    ...
    def make_queue(self):
        ...
        heading.addStretch()
        self.remove = button('', self.remove_folder)
        self.remove.setObjectName('queueDelete')
        self.remove.setIcon(tile_icon('trash'))
        self.remove.setIconSize(QSize(23, 20))
        ...
        self.add_button = button('+ Add media', primary=True)
        add_menu = QMenu(self.add_button)
        add_menu.addAction('Folder…', self.add_folders)
        add_menu.addAction('Individual videos…', self.add_video_files)
        self.add_button.setMenu(add_menu)
        self.add_button.setToolTip('Add a folder or select one or more individual videos')
    ...
    def refresh_models(self):
        ...
        if hasattr(self, 'visual_model_state'):
            ...
            self.visual_model_state.setText(('Installed · ' if ready else 'Included with 1.12 · approximately ') + size_text(size))
