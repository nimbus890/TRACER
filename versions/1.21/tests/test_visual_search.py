...
class VisualSearchTests(unittest.TestCase):
    ...
    def test_visual_index_samples_both_boundaries_and_uses_small_thumbnails(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            options = {**core.DEFAULTS, 'interval': 30}
            ...
            requested.clear()
            with patch('core.frame_at', side_effect=frame_at), patch('core.extract_frames',
                    return_value=[{'time': 0}, {'time': 3}, {'time': 7}]):
                indexer.index('source.mp4', root, 10, {**options, 'mode': 'scene'},
                              core.Control(), lambda *_: None)
            self.assertEqual(requested, [0.0, 3.0, 7.0, 10.0])
