...
class TimelineCanvas(QWidget):
    ...
    def mousePressEvent(self, event):
        ...
        if self.tool == 'ripple' and clip and event.button() == Qt.LeftButton:
            ...
            downstream = []
            for trk in self.sequence.get('tracks', []):
                for c in trk.get('clips', []):
                    if c['id'] != clip['id'] and float(c.get('start', 0)) >= float(clip.get('start', 0)):
                        downstream.append((c, float(c.get('start', 0)), float(c.get('end', 0))))
            self._drag_ripple = (pos.x(), index, clip, edge,
                                 float(clip.get('start', 0)), float(clip.get('end', 0)),
                                 float(clip.get('source_in', 0)), float(clip.get('source_out', 0)),
                                 downstream)
            self.update()
            return
    ...
    def mouseMoveEvent(self, event):
        ...
        if self._drag_ripple and (event.buttons() & Qt.LeftButton):
            press_x, t_idx, clip, edge, orig_s, orig_e, orig_in, orig_out, downstream = self._drag_ripple
            delta = (pos.x() - press_x) / self.pixels_per_second
            if edge == 'end':
                new_dur = max(0.05, (orig_e - orig_s) + delta)
                clip['end'] = round(orig_s + new_dur, 4)
                clip['source_out'] = round(orig_in + new_dur, 4)
                ripple_delta = new_dur - (orig_e - orig_s)
                for d_clip, d_s, d_e in downstream:
                    d_clip['start'] = round(max(clip['end'], d_s + ripple_delta), 4)
                    d_clip['end'] = round(d_clip['start'] + (d_e - d_s), 4)
            else:
                new_dur = max(0.05, (orig_e - orig_s) - delta)
                clip['start'] = round(max(0.0, orig_s + delta), 4)
                clip['source_in'] = round(max(0.0, orig_in + delta), 4)
            self._resize_canvas()
            self.update()
            return
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self._drag_ripple:
            _, _, clip, _, _, _, _, _, _ = self._drag_ripple
            self._drag_ripple = None
            self.clipTrimmed.emit(clip.get('id', ''))
            self.changed.emit()
        if self._drag:
