...
def validate_options(options):
    ...
    if not math.isfinite(float(o['visual_interval'])) or not 1 <= float(o['visual_interval']) <= 30:
        raise ValueError('Visual indexing interval must be between 1 and 30 seconds')
...
def signature(path, options):
    ...
    raw = json.dumps([str(p.resolve()).casefold(), st.st_size, st.st_mtime_ns, relevant, 2], sort_keys=True)
