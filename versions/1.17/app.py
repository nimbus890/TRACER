...
def paint_origin_icons(painter, origins, right, top, selected=False):
    ...
    colour = '#fff1e7' if selected else '#9dabb4'
    x = right - len(origins) * 18
    for origin in origins:
        tile_icon(kinds[origin], colour).paint(painter, x, top, 15, 15)
        x += 18


def search_origin_icon(origins):
    width = max(18, len(origins) * 18)
    pixmap = QPixmap(width, 18)
    pixmap.fill(Qt.transparent)
    painter = QPainter(pixmap)
...
class SearchResultDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        paint_origin_icons(painter, data.get('origins', ()), rect.right(), rect.top(), selected)
...
class OptionsDialog(QDialog):
    ...
    def values(self):
        return {**self.options, 'model': self.model.currentData(),
                ...
                'visual_index': self.visual_index.isChecked(), 'visual_interval': self.visual_interval.value(),
    ...
    def update_estimate(self):
        ...
        self.capture_mode_title.setEnabled(o['screenshots'])
        self.capture_mode_box.setEnabled(o['screenshots'])
...
class FootageSearchPage(QWidget):
    ...
    def seek_frame(self, item):
        ...
        self.player.setPosition(round(frame['time'] * 1000))
...
class ProjectsPage(LegacyProjectsPage):
    ...
    def build_editing_panel(self):
        ...
        self.scrub_frame = QLabel('Move the playhead to preview the sequence')
    ...
    def render_scrub_frame(self):
        ...
        size = self.scrub_frame.size()
        pixmap = QPixmap.fromImage(rendered).scaled(
            max(1, size.width()), max(1, size.height()), Qt.KeepAspectRatio, Qt.SmoothTransformation)
