...
win.grab().save(str(core.ROOT / 'verification' / 'paper-find-1.16.png'))
...
win.grab().save(str(core.ROOT / 'verification' / 'paper-find-narrow-1.16.png'))
...
win.grab().save(str(core.ROOT / 'verification' / 'paper-no-matches-1.16.png'))
...
win.projects_page.desk.apply_preset('Editing desk')
...
QTest.qWait(100)
win.grab().save(str(core.ROOT / 'verification' / 'storyline-1.16.png'))
...
print('1.16 acceptance passed: Queue startup, Find, offline/no-match, focus restore, 1100px layout, passage order, stacking, zoom sync, export dock, close.')
