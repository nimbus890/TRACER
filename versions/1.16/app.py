...
# The 1.16 desk uses restrained graphite surfaces and crisp, compact controls.
STYLE += '''
    ...
    def populate(self):
        ...
        for record in reversed(self.records):
            ...
            item.setData(Qt.UserRole + 1, {
                ...
                'hits': f'{direct} direct · {embedded} indirect' if query else '',
            })
...
class ProjectsPage(LegacyProjectsPage):
    ...
    def install_modular_workspace(self):
        ...
        self.desk = workspace.Workspace(self.window, 'storyline')
        ...
        self.desk.add_panel('export', 'Export Queue', self.export_panel)
        self.desk.configure('timeline', {
            'Editing desk': [(['media'], 270), (['preview', 'timeline'], 1050)],
            'Timeline focus': [(['media'], 240), (['timeline'], 1050)],
            'Preview on the left': [(['preview'], 360), (['timeline'], 950)],
            'Delivery desk': [(['preview', 'timeline'], 1060), (['export'], 250)],
        })
    ...
    def set_delivery_open(self, opened):
        if hasattr(self, 'desk'):
            self.desk.show_panel('export') if opened else self.desk.hide_panel('export')
...
class Window(QMainWindow):
    def __init__(self):
        ...
        side.addSpacing(10)
        version = label('Tracer · 1.16', 'subtle')
