...
Double-click **Launch TransPro.vbs** in this folder to open the current working build. The versioned standalone app remains in **dist/TransPro/TransPro.exe**.
...
The editable 1.6 source is preserved under **versions/1.6**. Its installer is intentionally deferred until an exportable build is requested; completed earlier installers remain in **release**.
...
4. Click **Process folders** and choose any combination of the three compact, opaque icon tiles above Start Processing. Hover an icon for its Transcript, Screenshots, or searchable Visual Index explanation; unrelated settings fade when that output is off.
5. For screenshots, choose a 0.1–30 second interval (type an exact value or use the slider), or choose scene changes. Choose adjustable-quality JPEG or lossless PNG.
6. Open **Advanced settings** only when you need to change the processing device, the default Standard 1280 px screenshot size, or choose among multiple installed Whisper models. A model selector is not shown when only one model is installed.
...
8. Open **Footage search** to search object keywords by foreground, midground, or background. Results appear as videos first. Select a video for the coarse index, then double-click a moment for a five-second detail strip.
9. Open **Video library** to search transcripts and keywords, play videos, browse screenshots, and open exported files. Choosing a video slides the browser away, leaving a transcript-first workspace, source preview, and full-width screenshot gallery. Drag the divider between transcript and preview left or right to suit the footage. Separate buttons open the generated results and original video's folder.
...
Search results are grouped by video to avoid flooding the screen with repeated frames. Selecting a video reveals its matching coarse moments. Double-clicking one creates a lazy contact strip at five-second intervals for that 30-second region; those detail frames are cached in the result folder.
...
By default, outputs live in a **TransPro Results** folder inside each selected folder, with a separate subfolder per video/run. Names include a source-path identifier and timestamp to avoid collisions. Original videos are not modified. Recursive scans exclude TransPro Results. An alternate destination groups results by source folder.
...
Model removals are recoverable: they move into data/removed-models. They do not free space until those backups are deleted. The app provides a button to open that folder.
...
- Keep the entire standalone TransPro folder together. Its data folder stores models and library history. The per-user installer preserves data on uninstall.

## Development

Use Python 3.12 on Windows x64. Create a virtual environment and install requirements.txt. Run `python app.py`. Build with `python build.py`, then compile TransPro.iss using Inno Setup 6.7 or newer. The Inno compiler is a build tool, not an app dependency; commercial use of that compiler may require a license.
