"""Isolated 1.23 journey checks and screenshots for Tracer 1.23."""
import copy
import os
...
# Ensure 1.23 modules are imported first
...
def main():
    ...
    with tempfile.TemporaryDirectory() as temporary, patch.object(core, 'DATA', Path(temporary)), patch.object(app.Window, 'first_setup', lambda self: None):
        ...
        # beta-off
        win.set_beta_features(False)
        win.select_collection(c['id'])
        settle()
        assert win.collection_page.paper_collection_button.isHidden()

        # beta-on
        win.set_beta_features(True)
        win.select_collection(c['id'])
        settle()
        assert not win.collection_page.paper_collection_button.isHidden()
        ...
        story.tool_controller.handle_key_press(event)
        settle()
        assert len(story.active_sequence()['tracks'][1]['clips']) == before + 1
    ...
    print('1.23 journey checks passed. Screenshots: ' + str(output))
