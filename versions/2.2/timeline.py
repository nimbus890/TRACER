...
from PySide6.QtGui import QColor, QDrag, QFont, QPainter, QPen, QBrush, QKeySequence, QPixmap, QPolygon, QCursor
    ...
    for kind in ('video', 'audio'):
        ...
        for track in (t for t in sequence.get('tracks', []) if t.get('type') == kind):
            ...
            for clip in sorted(track.get('clips', []), key=lambda c: c.get('start', 0)):
                ...
                clip_duration = round(float(clip.get('duration', float(clip['end']) - float(clip['start']))) * fps)
                ET.SubElement(item, 'duration').text = str(max(1, clip_duration))
                _rate(item, fps)
                ...
                default_out = source_in + float(clip['end']) - float(clip['start'])
                source_out = float(clip.get('source_out', default_out))
                ET.SubElement(item, 'in').text = str(round(source_in * fps))
                ET.SubElement(item, 'out').text = str(round(source_out * fps))
                ...
                _rate(file_node, fps)
                ET.SubElement(file_node, 'duration').text = str(max(1, clip_duration))
...
def collect_sequence_media(sequence, parent, progress_callback=None, cancel_check=None):
    ...
    sources = list(sequence_sources(sequence))
    valid_sources = []
    ...
    for source_path in valid_sources:
        ...
        def on_chunk(b):
            ...
            if progress_callback and total_bytes > 0:
                ...
                progress_callback(pct, source_path.name, copied_count, total_files)

        if progress_callback:
            pct = min(99, int(copied_bytes / max(1, total_bytes) * 100))
            progress_callback(pct, source_path.name, copied_count, total_files)

        try:
            core.copy_file_with_progress(source_path, destination, on_bytes=on_chunk, cancel_check=cancel_check)
            copied_count += 1
            replacements[str(source_path).casefold()] = str(destination)
            manifest.append({'source': str(source_path), 'collected': str(Path('Media') / name), 'status': 'copied'})
        except OSError as e:
            manifest.append({'source': str(source_path), 'collected': None, 'status': f'error: {e}'})

    if progress_callback and not (cancel_check and cancel_check()):
        progress_callback(100, 'Done', copied_count, total_files)
...
class MediaBinList(QListWidget):
    def startDrag(self, supported_actions):
        ...
        if not items and self.currentItem():
            items = [self.currentItem()]
        payloads = [item.data(Qt.UserRole) for item in items if item and item.data(Qt.UserRole)]
        if not payloads:
            return
...
class ProjectMediaTree(QTreeWidget):
    ...
    def startDrag(self, supported_actions):
        ...
        if not items and self.currentItem():
            items = [self.currentItem()]
        payloads = [item.data(0, Qt.UserRole + 1) for item in items if item and item.data(0, Qt.UserRole + 1)]
        if not payloads:
            return
        payload = payloads[0] if len(payloads) == 1 else payloads
        mime = QMimeData()
...
class TimelineCanvas(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._autoscroll_timer.setInterval(35)
        self._autoscroll_timer.timeout.connect(self._handle_autoscroll)
    ...
    def set_zoom(self, value):
        new_value = max(0.02, min(120.0, float(value)))
    ...
    def _snap_time(self, value, include_playhead=False, exclude_ids=()):
        ...
        threshold = 8.0 / max(0.02, self.pixels_per_second)
    ...
    def fit(self, available_width):
        ...
        target = max(0.02, (available_width - TRACK_HEADER - 32) / duration)
    ...
    def paintEvent(self, event):
        ...
        if not self.sequence:
            ...
            return
        duration = max(sequence_duration(self.sequence) + 10, (self.width() - TRACK_HEADER) / max(0.02, self.pixels_per_second))
        pps = max(0.02, self.pixels_per_second)
        tick_candidates = [0.5, 1, 2, 5, 10, 15, 30, 60, 120, 300, 600, 900, 1800, 3600, 7200]
        step = next((s for s in tick_candidates if s * pps >= 64), tick_candidates[-1])
        minor_step = step / 5 if step * pps >= 80 else step / 2 if step * pps >= 48 else None
        painter.setFont(QFont('Segoe UI', 8))
        # Minor ticks
        if minor_step and minor_step * pps >= 8:
            for i in range(int(duration / minor_step) + 2):
                t = i * minor_step
                mx = round(TRACK_HEADER + t * pps)
                if mx < TRACK_HEADER:
                    continue
                painter.setPen(QPen(QColor('#333330'), 1))
                painter.drawLine(mx, RULER_HEIGHT - 4, mx, RULER_HEIGHT)
        # Major ticks
        for i in range(int(duration / step) + 2):
            second = i * step
            x = round(TRACK_HEADER + second * pps)
            if x < TRACK_HEADER:
                continue
            ...
            painter.setPen(QColor('#99958e'))
            if second >= 3600:
                lbl = f'{int(second // 3600)}:{int((second % 3600) // 60):02d}:{int(second % 60):02d}'
            elif second >= 60:
                lbl = f'{int(second // 60)}:{int(second % 60):02d}'
            else:
                lbl = f'{second:.1f}s' if step < 1 else f'0:{int(second):02d}'
            painter.drawText(x + 3, 16, lbl)
        for index, track in enumerate(self.sequence.get('tracks', [])):
            ...
            # Hide / Mute icon on track
            is_audio = track.get('type') == 'audio'
            is_hidden = not track.get('visible', True) if not is_audio else track.get('muted', False)
            hide_color = QColor('#e06c6c' if is_hidden else '#7d8990')
            painter.setPen(QPen(hide_color, 1.35, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
            cx, cy = 20, y + 38
            if is_audio:
                painter.drawRect(cx - 5, cy - 3, 3, 6)
                painter.drawPolygon(QPolygon([QPoint(cx - 2, cy - 3), QPoint(cx + 2, cy - 6), QPoint(cx + 2, cy + 6), QPoint(cx - 2, cy + 3)]))
                if is_hidden:
                    painter.drawLine(cx - 6, cy + 5, cx + 5, cy - 6)
                else:
                    painter.drawArc(cx + 2, cy - 4, 4, 8, -60 * 16, 120 * 16)
            else:
                painter.drawArc(cx - 7, cy - 4, 14, 8, 30 * 16, 120 * 16)
                painter.drawArc(cx - 7, cy - 4, 14, 8, 210 * 16, 120 * 16)
            ...
            lock_color = QColor('#e59a59' if is_locked else '#586268')
            painter.setPen(QPen(lock_color, 1.35, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
            lx, ly = 44, y + 38
            painter.drawRoundedRect(lx - 5, ly - 1, 10, 7, 1, 1)
            if is_locked:
                painter.drawArc(lx - 3, ly - 7, 6, 7, 0, 180 * 16)
                painter.drawLine(lx - 3, ly - 3, lx - 3, ly - 1)
                painter.drawLine(lx + 3, ly - 3, lx + 3, ly - 1)
            else:
                painter.drawArc(lx - 4, ly - 7, 6, 7, 20 * 16, 170 * 16)
                painter.drawLine(lx - 4, ly - 3, lx - 4, ly - 1)
            for clip in track.get('clips', []):
    ...
    def mousePressEvent(self, event):
        ...
        if clip:
            ...
            if pos.x() < TRACK_HEADER and pos.y() >= RULER_HEIGHT:
                ...
                if 0 <= track_idx < len(self.sequence.get('tracks', [])):
                    self.selected_track_index = track_idx
                    track = self.sequence['tracks'][track_idx]
                    local_y = (pos.y() - RULER_HEIGHT) % TRACK_HEIGHT
                    if 10 <= pos.x() <= 30 and 26 <= local_y <= 50:
                        ...
                        self.update()
                        return
                    elif 34 <= pos.x() <= 56 and 26 <= local_y <= 50:
    ...
    def mouseMoveEvent(self, event):
        ...
        if self._scrubbing and event.buttons() & Qt.LeftButton:
            self.set_playhead((pos.x() - TRACK_HEADER) / max(0.02, self.pixels_per_second),
                              snap=True, notify=True)
    ...
    def _check_autoscroll(self, pos):
        ...
        vp_pos = vp.mapFromGlobal(self.mapToGlobal(pos.toPoint() if hasattr(pos, 'toPoint') else QPoint(int(pos.x()), int(pos.y()))))
        vp_w = vp.width()
        if vp_pos.x() > vp_w - 40:
            self._autoscroll_dx = max(4, min(20, (vp_pos.x() - (vp_w - 40)) // 2))
            if not self._autoscroll_timer.isActive():
                self._autoscroll_timer.start()
        elif vp_pos.x() < TRACK_HEADER + 40:
            self._autoscroll_dx = -max(4, min(20, (TRACK_HEADER + 40 - vp_pos.x()) // 2))
            if not self._autoscroll_timer.isActive():
                self._autoscroll_timer.start()
        else:
            self._stop_autoscroll()

    def _handle_autoscroll(self):
        scroll_area = self.parentWidget()
        while scroll_area and not hasattr(scroll_area, 'horizontalScrollBar'):
            scroll_area = scroll_area.parentWidget()
        if not scroll_area:
            ...
            return
        bar = scroll_area.horizontalScrollBar()
        bar.setValue(bar.value() + self._autoscroll_dx)
        if self._scrubbing:
            cursor_pos = self.mapFromGlobal(QCursor.pos())
            self.set_playhead((cursor_pos.x() - TRACK_HEADER) / max(0.02, self.pixels_per_second),
                              snap=True, notify=True)
    ...
    def keyPressEvent(self, event):
        command = None
        if event.modifiers() == Qt.ShiftModifier and event.key() in (Qt.Key_Delete, Qt.Key_Backspace):
            ...
            event.accept()
            return
        elif event.key() in (Qt.Key_Delete, Qt.Key_Backspace) and event.modifiers() == Qt.NoModifier:
            command = 'delete'
