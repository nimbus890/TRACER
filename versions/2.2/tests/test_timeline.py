...
class TimelineTests(unittest.TestCase):
    def test_old_project_is_migrated_with_sequence_tracks_and_export_queue(self):
        ...
        self.assertEqual(project['sequences'][0]['view'], {'playhead': 0.0, 'zoom': 10.0, 'scroll': 0})
    ...
    def test_linked_clips_move_together_and_locked_partner_blocks_the_edit(self):
        sequence = timeline.new_sequence('Linked edit')
        linked = 'pair-1'
        video = timeline.make_clip('interview.mp4', 8, start=3, linked=linked)
        audio = timeline.make_clip('interview.mp4', 8, start=3, linked=linked)
        sequence['tracks'][1]['clips'].append(video)
        sequence['tracks'][2]['clips'].append(audio)
        positions = {value['id']: (value['start'], value['end'])
                     for _, _, value in timeline.clip_group(sequence, video)}
        timeline.move_clip_group(sequence, video, positions, 7)
        self.assertEqual((video['start'], audio['start']), (7.0, 7.0))
        self.assertEqual((video['end'], audio['end']), (15.0, 15.0))
        sequence['tracks'][2]['locked'] = True
        self.assertTrue(timeline.clip_group_locked(sequence, video))

    def test_timeline_xml_write_is_atomic_and_leaves_no_temporary_file(self):
        with tempfile.TemporaryDirectory() as root:
            target = Path(root) / 'sequence.xml'
            timeline.write_final_cut_xml(timeline.new_sequence('Safe export'), target)
