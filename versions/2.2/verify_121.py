"""Isolated 1.21 journey checks and screenshots for Tracer 1.21 finishing release."""
import copy
import os
...
# Ensure 1.21 modules are imported first
...
def main():
    ...
    with tempfile.TemporaryDirectory() as temporary, patch.object(core, 'DATA', Path(temporary)), patch.object(app.Window, 'first_setup', lambda self: None):
        ...
        # 3. Paper Edit with 1.21 features
        win.select_collection(c['id'])
        ...
        assert 'Synced to Storyline' in paper.paper.status_line.text()
        ...
        # 4. Storyline with 1.21 6-tool suite
    ...
    print('1.21 journey checks passed. Screenshots: ' + str(output))
