...
def linked_document(state: dict, sequence: dict | None) -> dict | None:
    """Resolve a sequence's manuscript without relying on current page state."""
    document_id = (sequence or {}).get('paper_document_id')
    return next((document for document in state.get('paper_edits', [])
                 if document.get('id') == document_id), None)
...
def status_badge_text(document: dict) -> str:
    """Return a quiet, factual link summary for the manuscript header."""
    ensure_passage_model(document)
    ...
    state = 'Linked' if document.get('sequence_id') else 'Not linked'
    return f"{state} · {len(included)} passage{'s' if len(included) != 1 else ''} · {time_str}"
