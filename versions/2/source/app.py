...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
    ...
    QSizePolicy, QTabWidget, QGridLayout, QAbstractButton)
...
class OptionsDialog(QDialog):
    def __init__(self, options, folders, parent, batch_name='folder batch'):
        ...
        self.cursor_aura = CursorAura(self.process_card, self)
...
class LibraryPickerDialog(QDialog):
    ...
    def __init__(self, records, parent=None):
        ...
        beta = parent and hasattr(parent, 'window') and parent.window.state.get('settings', {}).get('beta_features', False)
...
class ProjectsPage(LegacyProjectsPage):
    ...
    def build_media_panel(self):
        ...
        view_toggle_row.setContentsMargins(2, 2, 2, 2)
        view_toggle_row.setSpacing(6)
        view_toggle_row.addStretch()
        view_toggle_row.addWidget(label('List', 'subtle'))
        self.media_view_toggle = ToggleSwitch()
        self.media_view_toggle.setToolTip('Toggle between List and Grid view')
        self.media_view_toggle.setAccessibleName('Media view toggle')
        is_grid = bool(getattr(self.window, 'state', {}).get('settings', {}).get('sequence_media_grid', False))
        self.media_view_toggle.setChecked(is_grid)
        self.media_view_toggle.toggled.connect(self.set_media_view_mode)
        view_toggle_row.addWidget(self.media_view_toggle)
        view_toggle_row.addWidget(label('Grid', 'subtle'))
        ll.addLayout(view_toggle_row)
        if is_grid:
    ...
    def build_editing_panel(self):
        ...
        tool_grid.setHorizontalSpacing(2)
        ...
        track_actions = (
            ...
            ('toggle', 'Show/hide the selected video track · mute/unmute audio track', lambda: self.sequence_command('toggle_track')),
        ...
        for icon_name, tip, callback in track_actions:
            ...
            bottom_column += 1
        toolbar_layout.addLayout(tool_grid)
        toolbar_layout.addStretch()

        self.snap = button('')
        self.snap.setObjectName('timelineTool')
        self.snap.setIcon(workspace.icon('snap'))
        self.snap.setIconSize(QSize(18, 18))
        self.snap.setFixedSize(32, 30)
        ...
        right_controls.addWidget(self.snap)
